...
manager (its HTTP surface is owed with the portal's billing pages, #41). A payment method is a token at
the PSP, and commitments are M3.
...
more, takes a number and stores a document nothing can change again. **Gap-free numbering per issuer**
...
`InvoicingTests.TenConcurrentFinalizationsGetTenConsecutiveNumbers`). The issuer is configuration
...
| `billing-http-surface` | The billing account, its invoices and credit notes are grain calls with no route | They are the portal's billing pages' (#41), with the account's own ReBAC question — who may see a tenant's invoices is not a resource-group reader |
