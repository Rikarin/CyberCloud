...
| `billing-http-surface` | The billing account, its invoices and credit notes are grain calls with no route — configuring the account, attaching subscriptions, reading invoices and issuing credit notes | They are the portal's billing pages' (#41), with the account's own ReBAC question — who may see a tenant's invoices is not a resource-group reader |
