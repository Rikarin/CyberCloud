...
`expiresOn`: read at the top level only, a `GET` sent back as a `PUT` dropped the end and made the grant
permanent (`RoleAssignmentTests.AGetSentBackAsAPutKeepsTheEndItRendered`). After the instant, `GET` is the canonical `404`, the collection omits the row, and every
