...
| **Reconciler** | `k3s` in Testcontainers, real API server, real SSA — and, since 2026-09-15, a kubelet, so an operator `charts/bundle/install.sh` installs can run under test | Every PR, < 15 min | All pass; `Test` prints how many cluster-backed cases ran and fails a run in which a cluster-holding suite skipped more than it ran beside a Docker endpoint (`Build.Test.cs § ReportClusterBackedCases`) |
