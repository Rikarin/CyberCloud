...
| **File** | ~~`CyberCloud.Storage/fileShares`~~ `CyberCloud.Storage/accounts/fileShares` | ~~SeaweedFS FUSE/NFS~~ SeaweedFS CSI (FUSE), or LINSTOR RWX + an NFS server | Mounted by ~~VMs and~~ pods |
...
> carries nine named debts. The three corrections that are this document's to own:
>
...
>   and removed by its *last*, because a driver is a controller Deployment plus two DaemonSets on every
>   node and an account with no shares should not pay for one. What that costs — unmetered pods per
>   account, and a driver whose resource-id label names whichever share applied it last — is
>   `one-driver-per-account` and `the-driver-carries-one-shares-labels`.
>
