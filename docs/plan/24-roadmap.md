...
```text
✔ Generated surfaces  Enforced  47 resource type(s) over 2 OpenAPI document(s), 14 derived file(s) —
...
✔ Generated SDK compiles  Enforced  1 api-version file(s) declaring 566 type(s), each compiled on its
  own against CyberCloud.Sdk — 408 partial member(s) accepted as declared-but-not-implemented
```
...
and the what-if's content and result; get, get by name, list, create, update, delete and what-if. Both
figures above are re-read from the run on the merge of #28's scale sets and container groups into
master (2026-09-24), not carried over from either side of it.
...
| Portal | Cost analysis, metrics explorer, log search, identity admin | 2.3 | #41 | ◐ **All four surfaces have landed under #41, identity admin in part, each from its own branch; the row stays ◐ for what each still owes.** *The metrics explorer and log search* landed 2026-09-23, with a resource-graph explorer beside them — [20 § The pages that are not generated](20-portal.md). Both explorers read a Monitor workspace through three read actions on the workspace type, `queryMetrics`, `listMetricLabels` and `searchLogs`, checked with `read`, answered under the workspace's own `accountID` and database and proved against a real VictoriaMetrics cluster and ClickHouse with a cross-tenant refusal — [16 § Querying a workspace](16-observability.md). The type list confirms nothing here: the explorers are actions and pages, not types. What is owed is at `charts/managed/monitor-workspace/conformance.yaml § owed`, from `query-responses-are-undeclared` to `explorers-are-wired-on-a-laptop-only`. ⚠ **Landed is not deployed:** only the AppHost configures the gateway's stores, and only the log half, so the metrics explorer refuses by name everywhere; and the `accountID` it reads under is the workspace GUID folded to 32 bits, which two workspaces can share — #41's review added the ledger that refuses the second workspace at create and refuses its reads, and what is left is `accountid-is-folded-not-allocated`. *Identity admin*, three of its five nouns, landed 2026-09-24 — no resource type, so the type list can't show it: members, invitations and registered applications for a tenant owner, and the signed-in person's own sessions, over an identity administration API at the gateway under the reserved `CyberCloud.Identity` namespace ([10 § Shape](10-gateway-and-api.md), [11 § Sign-up and tenant creation](11-identity.md)) and a per-tenant directory index, the 32nd grain-key shape. A confidential client's secret is now minted by the platform and kept as a digest, so a client registered from the portal signs people in end to end. ⚠ Groups, MFA administration and sign-in logs are not landed — [20 § The pages that are not generated](20-portal.md) lists each with the rest of what is owed. *Cost analysis* landed 2026-09-24 at `subscriptions/{s}/cost` and `…/resourceGroups/{g}/cost`: cost over time by service, resource group, resource or meter for a period in the URL, a stacked daily chart on `@xui/echarts` beside the table that is its accessible copy, a month-end forecast linear on the trailing seven days, a group's budgets with the thresholds that fired, and the tenant's invoices with their lines — [20 § The pages that are not generated](20-portal.md). ⚠ **The chart library is over the route budget by construction**, so it has a ceiling of its own: ECharts' core alone is 129.3 KB gzipped against 120, and the portal's build is one deferred chunk against 180, a ceiling #41 proposed and nobody has yet signed off ([20 § Performance budget](20-portal.md), `chart-library-ceiling`) — 172.8 KB for the cost page alone, 177.3 KB since the explorers and the cost page draw through one engine. What cost analysis still owes is [20 § What is owed](20-portal.md). ⚠ The 2.3 is four pages and no EM is claimed: the cost page is [20](20-portal.md)'s 0.6 less by tag and the per-resource figure on the blade |
...
⚠ **On "28 resource types": there are 47 today**, and twenty-eight of them are this phase's — the `Data`
...
`Compute/virtualMachines`, `Compute/disks`, `Compute/images`), the four of 2026-09-23
(`Resources/deployments` from #39, `Billing/budgets` from #38, `Monitor/workspaces/components` from #32
and `Mail/domains/mailboxes` from #34), and the two #28 merged last on 2026-09-24
(`Compute/virtualMachineScaleSets`, `ContainerInstance/containerGroups`), then #31's
`Network/virtualNetworks/applicationGateways` — 4 + 1 + 4 + 1 + 4 + 7 + 4 + 2 + 1, summed at the merges, because master's own sentence had said twenty-two over a table
row of twenty-five. The other
nineteen are counted, phase by phase, in [§ What has landed](#what-has-landed--recounted-2026-09-23) —
which is also where to see that two of the 47 belong to phase 4 and one is phase 1's deliberately
trivial sample. ⚠ **The count passed the criterion and the criterion is not met**: "28 resource
types" was written as a proxy for a catalogue, sixteen of the 47 are children of families, and the
...
`Resources/deployments` on 2026-09-23, recounting in the same change, and #38, #32 and #34 published
three more beside it. #28's scale sets and container groups recounted themselves to 41 on their own
branch against a master that had 39, and master had moved on without them — so the merge of that
branch took neither side's number and re-ran the command, and 46 was its answer; #31's application gateways, recounted on their branch to 45 against an older master, made it 47 at their merge, counted rather than picked. That is worth *establishing* rather than
...
```text
✔ Generated surfaces  Enforced  47 resource type(s) over 2 OpenAPI document(s), …
```
...
All 47, against the phase that planned them. ⚠ **Written out in full, with only the `CyberCloud.`
...
| 3 — M2 | `DocumentDB/accounts`, `Messaging/rabbitmqClusters`, `Messaging/kafkaClusters`, `Analytics/clickhouseClusters`, `Mail/domains`, `Mail/domains/mailboxes`, `Communication/services`, `Communication/services/channels`, `Communication/services/templates`, `Communication/services/suppressions`, `Monitor/workspaces/alertRules`, `Storage/accounts/fileShares`, `Network/virtualNetworks/natGateways`, `ContainerService/connectedClusters`, `ContainerRegistry/feeds`, `Network/virtualNetworks/peerings`, `RecoveryServices/vaults`, `Monitor/workspaces/collectors`, `Dashboard/grafanas`, `Compute/virtualMachines`, `Compute/disks`, `Compute/images`, `Resources/deployments`, `Billing/budgets`, `Monitor/workspaces/components`, `Network/virtualNetworks/applicationGateways`, `Compute/virtualMachineScaleSets`, `ContainerInstance/containerGroups` | 28 |
...
row in the catalogue and is not in the 47 — because tenants, subscriptions and resource groups are
...
which gives **89.1**. Against that sit 11.3 + 11.4 + 1.8 = **24.5 EM of rows whose every named type is
...
does not say which of its rows were on the critical path in the first place. 89.1 − 24.5 = **64.6** is
therefore the floor and 89.1 the ceiling.

⚠ **That figure — 17.5 until 2026-09-15, 19.5 until 2026-09-18, 20.3 until 2026-09-23, 20.7 until 2026-09-24, 24.5 since — said "rows that are finished" until 2026-09-06, and this document's own annotations do not
...
is truly finished can only push the result *down*, so 64.6 remains a valid lower bound — it is simply a
...
anything does, and either way it is the reason 24.5 EM of completed work cannot simply be subtracted.
...
At 4–5 engineers that is **roughly 13–22 months** — 64.6 ÷ 5 = 12.9 at the fast end, 89.1 ÷ 4 = 22.3 at
...
1. **That the operator selections in [12](12-managed-data-services.md) hold up without a fork —
   holding, with one exception that is worse than a fork.** All 34 charts under `charts/managed/` carry
