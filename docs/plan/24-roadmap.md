...
| **Mail** | Postfix/Dovecot/Rspamd, domains, mailboxes, deliverability, minimal webmail | 3.5 | #34 | ◐ `Mail/domains` published (#34, 2026-09-12) with the chart under `charts/managed/mail`, and `Mail/domains/mailboxes` (#34, 2026-09-23) under `charts/managed/mail-mailbox`, with the domain's `dnsRecords` and `verify` actions and the sending gate they decide. ⚠ Four nouns of five, and the fifth — webmail — is the one the type list could never show. What keeps the row ◐ is not in the list either: the shared front doors, the outbound pool and its warm-up, and a Postfix image nothing publishes — `charts/managed/mail/conformance.yaml § owed` |
...
none of their numbers and re-ran the command a second time; 39 was right until #34 published
`Mail/domains/mailboxes` on 2026-09-23, recounting in the same change; 40 is right. That is worth *establishing* rather than
...
| 3 — M2 | `DocumentDB/accounts`, `Messaging/rabbitmqClusters`, `Messaging/kafkaClusters`, `Analytics/clickhouseClusters`, `Mail/domains`, `Mail/domains/mailboxes`, `Communication/services`, `Communication/services/channels`, `Communication/services/templates`, `Communication/services/suppressions`, `Monitor/workspaces/alertRules`, `Storage/accounts/fileShares`, `Network/virtualNetworks/natGateways`, `ContainerService/connectedClusters`, `ContainerRegistry/feeds`, `Network/virtualNetworks/peerings`, `RecoveryServices/vaults`, `Monitor/workspaces/collectors`, `Dashboard/grafanas`, `Compute/virtualMachines`, `Compute/disks`, `Compute/images` | 22 |
...
| 3 — M2 | 28 | 38.4 | **7.2 ✅** | The whole `Data` row (4.4), the whole `Communication` row (2.0, 2026-09-15), and the `NAT gateways, peering` line of the `Network` row (0.8, 2026-09-18) — the one line of [14 § Effort](14-networking.md) that row has a per-noun split for, and both nouns are published. ⚠ `Mail`'s ◐ is counted as zero: four nouns of five are published and the row has no defensible split — [17 § Effort](17-communication-and-email.md) prices the pieces by work, not by noun, and its largest open piece (outbound pools, 0.8) is no noun at all. `Observability`'s ◐ is counted as zero for the same reason — alerts are one noun of four, and the 3.0 has no per-noun split to read |
