...
| Portal M1 subset | 5.0 | [20](20-portal.md) | ◐ no resource type. #22: the generated form renderer, create/edit, the resource blade and list, the operation view, and the subscription and resource-group pages exist over the generated client. ⚠ Quota/usage and the cloud-shell surface wait on endpoints (docs/plan/22, an `xterm.js` host); the role-assignments page waits on a client for the write path #70 landed and the collection `GET` #86 added — both routed explicitly and in no generated surface, which is #63's question ([10](10-gateway-and-api.md) § Shape); the region/cluster/storage-class/subnet pickers wait on list endpoints |
