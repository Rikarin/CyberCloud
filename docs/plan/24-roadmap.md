...
```text
✔ Generated surfaces  Enforced  35 resource type(s) over 2 OpenAPI document(s), 14 derived file(s) —
...
| Managed identity + token exchange | 1.2 | [11](11-identity.md) | ⚠ **Not shipped, and the type list says so rather than staying silent:** [01](01-azure-parity-catalogue.md) names `CyberCloud.ManagedIdentity/*` and nothing under it is among the 35. ⚠ **The one row that drifts the other way:** the catalogue verdicts managed identities **M2**, so this row is scheduled a phase *earlier* than the catalogue asks rather than having landed a phase early |
...
| Vault (OpenBao) | 2.0 | [18](18-security-vault-and-malware-scan.md) | ⚠ **Not shipped, and here the type list says so positively rather than saying nothing:** [01](01-azure-parity-catalogue.md) names `CyberCloud.KeyVault/vaults` as an M1 type at this row's 2.0 EM, and it is not one of the 35 |
...
| Compute | VMs + disks + images, scale sets, container instances | 3.8 | #28 | ◐ `Compute/virtualMachines`, `Compute/disks` and `Compute/images` published (#28, 2026-09-17) on KubeVirt and CDI, with the charts under `charts/managed/virtual-machine`, `charts/managed/disk` and `charts/managed/image` — the VM noun of [13 § Virtual Machines](13-compute-vm-containers.md): a size from a closed set that is what the guest gets and what quota reserves, a root disk cloned from an image in the same resource group, disks by name, a subnet by the Network family's own object name, cloud-init as a vault handle, and `start`/`stop`/`restart` as the first handlers in the tree that write a cluster. ⚠ **The first three nouns of five, and the first family whose webhook half is measured rather than owed**: `test/CyberCloud.Bundle.Cluster.Conformance § KubeVirtOnAnEmptyCluster` installs CDI and KubeVirt through `install.sh`, imports an image and applies the machine against the real webhooks — and the guest reports `Running` under KVM in 46 seconds, on the k3s-in-Docker lane #95 said had no `/dev/kvm` (it has: WSL2 nests virtualization and the k3s container is privileged). What is not reached is the guest itself — console, cloud-init, a disk inside — `charts/managed/virtual-machine/conformance.yaml § owed`, `the-guest-is-not-reached`. Scale sets and container instances are not types; `charts/managed/virtual-machine/conformance.yaml § owed`, `scale-sets-are-not-landed` and `container-instances-are-not-landed`, say what each waits for. ⚠ The 3.8 is the row's whole price and no EM is claimed: doc 13 prices the VM noun at 3.0 and what landed is its core |
...
⚠ **On "28 resource types": there are 35 today**, and seventeen of them are this phase's — the `Data`
...
`ContainerService/connectedClusters` and `ContainerRegistry/feeds`), and the `Compute` row's three
(#28, 2026-09-17). The other
eighteen are counted, phase by phase, in [§ What has landed](#what-has-landed--recounted-2026-09-17) —
which is also where to see that two of the 35 belong to phase 4 and one is phase 1's deliberately
trivial sample. ⚠ **The count passed the criterion and the criterion is not met**: "28 resource
types" was written as a proxy for a catalogue, nine of the 35 are children of families, and the
...
master that had 23 — the merge took none of their numbers and re-ran the command; 32 was right
until #28 published the three `Compute` types on 2026-09-17, recounting in the same change; 35 is
...
```text
✔ Generated surfaces  Enforced  35 resource type(s) over 2 OpenAPI document(s), …
```
...
All 35, against the phase that planned them. ⚠ **Written out in full, with only the `CyberCloud.`
...
| 3 — M2 | `DocumentDB/accounts`, `Messaging/rabbitmqClusters`, `Messaging/kafkaClusters`, `Analytics/clickhouseClusters`, `Mail/domains`, `Communication/services`, `Communication/services/channels`, `Communication/services/templates`, `Communication/services/suppressions`, `Monitor/workspaces/alertRules`, `Storage/accounts/fileShares`, `Network/virtualNetworks/natGateways`, `ContainerService/connectedClusters`, `ContainerRegistry/feeds`, `Compute/virtualMachines`, `Compute/disks`, `Compute/images` | 17 |
...
row in the catalogue and is not in the 35 — because tenants, subscriptions and resource groups are
