...
| `Microsoft.Azure.Kusto.Language` | 12.4.1 | `CyberCloud.ResourceGraph` | **Added 2026-09-18 (#54).** Microsoft's KQL parser and binder; the resource graph API takes a KQL subset and translates it to ClickHouse SQL ([08 § The resource-graph projection](08-resource-manager.md)), and writing a KQL parser here would be a compiler we would own. Newest stable, no dependencies. ⚠ **The licence is not the Apache-2.0 of its source.** The GitHub repository is Apache-2.0; the NuGet artefact ships `EULA-agreement.txt` — the Microsoft Software License Terms for the Azure Data Explorer SDK — with `requireLicenseAcceptance`. ADR-011 guards against a licence that forbids offering the software as a service, and ⚠ **this EULA has one, narrower than SSPL's** — the first record of this row said it had none, a misquotation the #54 review found. §4(e): you will not *"provide the software as a stand-alone hosted solution for others to use"*. The reading that keeps the package: an embedded parser translating a tenant's KQL inside the gateway is not a stand-alone hosted solution, and the platform is the *"significant primary functionality"* §2(b)(i) asks for. §2(b)(ii)–(iii) attach the day the gateway image is shipped to a self-hoster — the platform's terms must then *"protect it and Microsoft at least as much as this agreement"* and the platform indemnifies Microsoft — and nothing writes those terms yet. `Directory.Packages.props` quotes the three clauses and the exit (build the Apache-2.0 source), which closes both |
...
⚠ **CORRECTED A FOURTH TIME, 2026-09-15 (#87): the framework is at 22.1.4, the tooling is at
22.1.6, and strict peers had never been on.** The framework — `@angular/*`, `@angular/compiler-cli`,
...
1. **The tooling cannot share the framework's number, and xUI's does not either.** `@angular/cli`
   22.1.0 through 22.1.4 depend on `listr2@10.2.2` and on `@listr2/prompt-adapter-inquirer@4.2.4`,
   which peers `listr2` at exactly `10.2.1`; under strict peers that resolution is
   `ERR_PNPM_PEER_DEP_ISSUES`. 22.1.5 is the first consistent release, and xUI's own tag pins
   `@angular/cli 22.1.6` and `@angular/ssr 22.1.6` next to its 22.1.4 framework. The "one number to
