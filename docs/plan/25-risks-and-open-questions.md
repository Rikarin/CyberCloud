...
| 3 | ~~**LINBIT contract for LINSTOR/DRBD?**~~ (ADR-011) | — | ✅ **CLOSED 2026-08-20 — no contract. The platform runs LINSTOR and DRBD unsupported, the way Cozystack does.** ⚠ **The row was filed under a licence audit and was never a licence obligation**, which is why it sat open for nine days looking like a thing the platform owed. The licence question was answered before the row was written: [ADR-011](02-technology-decisions.md)'s own footnote says *"Running them is fine"*, because GPL-2 and GPL-3 restrict **distribution** and the platform runs this software rather than shipping it. What was actually being priced is **support and indemnity** on a synchronous block-replication layer under customer data — a commercial relationship somebody may buy, not a condition of use. Named correctly, it is answerable without a quote, and the answer is no. ⚠ **What makes it affordable is a different decision, not an absence of one:** [ADR-020](02-technology-decisions.md) makes **Talos Linux** the node OS, so DRBD arrives as a signed system extension assembled by Image Factory rather than a module built from source on each node at boot — which removes the failure mode support would most often be called about. And the bundle's default storage class is **single-replica and local with no DRBD in it at all** (`charts/bundle/openebs-localpv/`), so the replicated path is a switch thrown deliberately rather than a default nobody chose. The GA condition is unchanged and now lives where it can be acted on — [24 § The replicated-storage switch](24-roadmap.md) |
...
6. **xUI is consumed from npm at its published version, not from the local checkout.** ADR-017's
   ...
   #87 moved the framework to 22.1.4, the version the 3.0.0 bundles were compiled with; ADR-017
   carries the corrections and portal/README.md § The Angular pin the measured table. ADR-017's
