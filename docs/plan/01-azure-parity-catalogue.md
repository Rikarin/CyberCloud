...
| ARM templates / Bicep | Declarative deployment of a resource graph | `CyberCloud.Resources/deployments` | **M2** | A JSON template with `dependsOn`, evaluated into a DAG by a deployment grain. **No Bicep** — a second language is a compiler we would own. ⚠ Landed with #39 (2026-09-23), and the DAG is driven by the deployment's own **operation** grain rather than by a deployment grain: a parent operation with a child operation per resource, each written through the write path as the deployment's creator, a closed expression set, a `whatIf`, and rollback recorded rather than performed — [08 § Long-running operations](08-resource-manager.md) says why the grain the row names would have been a second copy of the operation grain |
| Management Groups | Subscription grouping for policy and RBAC | `CyberCloud.Management/managementGroups` | **M2** | A tree above subscriptions; ReBAC inherits down it. ⚠ Landed with #39 as a *scope path*, `/tenants/{t}/managementGroups/{name}`, and not as a typed resource under this namespace — the same shape subscriptions took, and for the same reason ([06 § The hierarchy](06-tenancy-and-resource-model.md)). The RBAC half is built, and since #46 so is the policy half: an assignment at a group reaches every subscription and resource group beneath it ([08 § Policy](08-resource-manager.md)) |
...
| Azure Policy | Deny/audit/modify rules over resource shape | `CyberCloud.Policy/policyDefinitions` | **M3** | JSON-Logic-shaped conditions over the resource body, evaluated in the write path before the provider is called. ⚠ Landed with #46 as *scope extensions* — `{scope}/providers/CyberCloud.Policy/{policyDefinitions,policyAssignments,policyStates}` — and not as a typed resource in the registry: an assignment sits above a resource group, which no registry type can, and a rule is a recursive tree, which the registry's flat schema deliberately cannot express. The same shape role assignments took ([08 § Policy](08-resource-manager.md)) |
...
| Azure Resource Graph | KQL over all your resources | `CyberCloud.ResourceGraph` query API | **M3** | A per-tenant projection into ClickHouse fed by the resource change stream. ~~Not KQL — SQL over a flattened table~~ ⚠ **Corrected 2026-09-18 (#54): a KQL subset, translated to SQL — never SQL exposed.** The person typing the query knows Azure's `resources` table, and a SQL surface over the flattened table would have put ClickHouse's own grammar and a tenant's raw column names on the wire. Microsoft's parser (`Microsoft.Azure.Kusto.Language`) parses and binds against one declared `resources` table; `KqlTranslator` emits one parameterised ClickHouse statement with the caller's access ANDed in, and refuses anything outside the subset by name — [08 § The resource-graph projection](08-resource-manager.md) carries the subset and the address |
...
| Application Gateway / Front Door / WAF | `CyberCloud.Network/applicationGateways` | **M2** | ~~Envoy Gateway (Gateway API) + Coraza WAF~~ ⚠ **HAProxy + Coraza SPOA (#31)**, shipped as `virtualNetworks/applicationGateways` — [14 § Application gateway](14-networking.md) has why. Front Door's *global* half is anycast and PoPs — declined as a topology, offered as a feature name only where it is honest | 2.0 |
...
| Application Insights | ⊂ workspaces | **M2** | OTLP ingest, a per-tenant ClickHouse database, and the trace/exception views. ⚠ Landed 2026-09-23 (#32) as `CyberCloud.Monitor/workspaces/components` — a connection string and five views over the workspace's traces and logs; [16 § Application views](16-observability.md) says what it leaves owed |
| **OTel Collector as a service** | `CyberCloud.Monitor/collectors` | **M2** | In the brief. A tenant-owned collector deployment with a validated pipeline config — this is the correct primitive and Azure has no equivalent. ⚠ Landed 2026-09-17 (#32) as `CyberCloud.Monitor/workspaces/collectors` — a child of the workspace, with the configuration rendered by the platform rather than validated from the tenant; [16 § OTel Collector as a service](16-observability.md) says what that leaves owed | 1.0 |
| Managed Grafana | `CyberCloud.Dashboard/grafanas` | **M2** | grafana-operator, per-tenant instance, datasources pre-wired to that tenant's workspace and nothing else — ⚠ landed 2026-09-17 (#32) as one Deployment of upstream's image by digest rather than the operator, in the Monitor family's assemblies under this namespace; [16 § Managed Grafana](16-observability.md) says why | 0.8 |
