...
/// <summary>
///     docs/plan/23 § The chaos invariants, 7:
///     <i>
...
///         ⚠
///         <b>
///             Measured at three silos and one version, and reported as ○ for that reason unless it
///             fails.
///         </b> The machine hosts three silos, not thirty, and there is one build of the silo,
...
public sealed class Invariant7RollingRestartTests(ChaosTopology topology) {
    ...
    public async Task Invariant7_ARollingRestartOfEverySecondarySiloUnderLoadFailsNoTenantRequest() {
        ...
        var load = Enumerable.Range(0, Readers)
                        ...
                        while (!stop.IsCancellationRequested) {
                            ...
                            try {
                                ...
                                if (reader == 0 && i % 5 == 0) {
                                    ...
                                    if (write.IsFailure) {
                                        failures.Add(
                                            $"create roll-new-{i}: {write.Error!.Code} — {write.Error.Message}"
                                        );
        ...
        try {
            foreach (var silo in topology.SecondarySilos) {
                ...
                output?.WriteLine(
                    $"[{rolling.Elapsed.TotalSeconds:F1}s] {silo} → {replacement} in {step.Elapsed.TotalSeconds:F1} s (graceful stop {stopTook.TotalSeconds:F1} s)"
                );
        ...
        var numbers = new Dictionary<string, double>(StringComparer.Ordinal) {
            ...
            ["restartSecondsMax"] =
                Math.Round(restarts.Count == 0 ? 0 : restarts.Max(static x => x.Took.TotalSeconds), 1),
            ["gracefulStopSecondsMax"] = Math.Round(
                restarts.Count == 0 ? 0 : restarts.Max(static x => x.StopTook.TotalSeconds),
                1
            )
        };

        var detail =
            $"{restarts.Count} of {topology.Silos.Count} silos restarted gracefully one at a time (slowest {(restarts.Count == 0 ? 0 : restarts.Max(static x => x.Took.TotalSeconds)):F1} s, of which the graceful stop "
            + $"{(restarts.Count == 0 ? 0 : restarts.Max(static x => x.StopTook.TotalSeconds)):F1} s) "
        ...
        notConverged.ShouldBeEmpty(
            "creates accepted during the rolling restart did not converge: " + string.Join("; ", notConverged)
        );
