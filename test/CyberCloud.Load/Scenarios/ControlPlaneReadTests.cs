...
/// <summary>
///     docs/plan/23 § The load scenarios, row 1:
///     <i>
///         10 000 tenants, 1 000 000 resources, 5 000 rps
///         reads → control-plane read p99 &lt; 25 ms
...
public sealed class ControlPlaneReadTests(LoadTopology topology) {
    ...
    public async Task ControlPlaneReadsAtATenthOfTargetRateThroughTheRealGateway() {
        ...
        var distribution = await driver.RunAsync(
            async (i, ct) => {
                ...
                using var response = await topology.Http.SendAsync(
                    topology.Get(caller, world, name),
        ...
        distribution.AchievedRate.ShouldBeGreaterThan(
            Rate * 0.9,
            $"the driver reached {distribution.AchievedRate:F0} rps of the {Rate:F0} asked for, so this is not the row's rate."
        );
        distribution.Errors.ShouldBeLessThan(
            (int)(0.005 * (distribution.Samples + distribution.Errors)) + 1,
            "more than 0.5 % of reads failed: "
            + string.Join(", ", driver.Failures.Select(static x => $"{x.Value} × {x.Key}"))
        );
    }
}
