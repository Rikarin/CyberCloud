...
///         order is the subject. <c>ClusterConformanceHarness</c> installs an open-schema
///         CustomResourceDefinition stub for every custom kind a case renders that the API server
///         does not already serve, and <c>install.sh</c>'s helm install of cloudnative-pg would then
///         find <c>clusters.postgresql.cnpg.io</c> already present and refuse to adopt it. So the
///         installer runs first, the harness starts second and finds the operator's real definitions
///         served, and the stubs it does install are for the two Kube-OVN kinds no bundle component
///         on this lane can serve — see the test.
///     </para>
///     <para>
...
///         installs an open-schema definition for each kind and the two objects are admitted,
///         labelled and readable — and route nothing. What the network half of this test proves is
...
public sealed class M1StoryOnAFreshCluster(M1StoryClusterFixture cluster) : IClassFixture<M1StoryClusterFixture> {
    ...
    public async Task AVpcAndAPostgresServerComeUpOnAFreshClusterAndListKeysOpensAConnection() {
        ...
        (await ReadClusterScopedAsync(raw, VirtualNetworks.VpcRef(ns, NetworkName), token)).ShouldNotBeNull(
            ...
            + "is on the API server. ⚠ It is admitted against the harness's open-schema stub, because this "
            + "lane has no Kube-OVN — docs/plan/23 § The lane that needs a kubelet."
        );
