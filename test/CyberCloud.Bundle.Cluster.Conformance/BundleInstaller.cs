...
public static class BundleInstaller {
    ...
    ///     answer for exactly those three rows — through
    ///     <see cref="RunAsync(string, string?, CancellationToken, TimeSpan?)" />'s own parameter. So
    ///     the helm-only call sites keep the twelve minutes that hide nothing, and the runs that can
    ///     pay an establishment wait or a second helm timeout are the ones allowed to.
    /// </remarks>
    ...
    /// <summary>
    ///     What the run holding openebs-localpv, CDI and KubeVirt — one helm row and two
    ///     <c>manifest:</c> rows — is bounded by: helm's 10 m, then 5 m of establishment wait and
    ///     10 m of <c>waitFor:</c> per manifest row, plus <see cref="Margin" />.
    /// </summary>
    /// <remarks>
    ///     <see cref="BudgetFor" />'s answer for those three components, kept under a name because
    ///     <c>KubeVirtOnAnEmptyCluster</c> is the one run in the tree that pays it and its project
    ///     names the run by what it installs. 42 m on the tree of 2026-09-18; it moves when a
    ///     <c>waitFor:</c> block does. Longer than the script's own worst case by the same rule
    ///     <see cref="Budget" /> follows, so that a slow <c>KubeVirt</c> resource surfaces as
    ///     <c>kubectl wait</c>'s own timeout rather than as this harness killing bash. Measured on
    ///     2026-09-17 on a fresh k3s with an empty image cache, the run takes minutes and not tens of
    ///     them — <c>KubeVirtOnAnEmptyCluster</c> records what.
    /// </remarks>
    public static TimeSpan ManifestBudget => BudgetFor([OpenEbsLocalPvComponent, CdiComponent, KubeVirtComponent]);
    ...
    ///     <see cref="Budget" /> — one helm row's worth. A run that selects more than one installing
    ///     component passes <see cref="BudgetFor" />, and the one run that selects a <c>manifest:</c>
    ///     row passes <see cref="ManifestBudget" />, which is the same arithmetic under a name.
    /// </param>
    /// <remarks>
    ...
    public static async Task<Run> RunAsync(
        ...
        var start = new ProcessStartInfo(Bash ?? "bash") {
            ...
            // ⚠ UTF-8, STATED, because the script's output is and the default is not. install.sh
            // prints `── phase 5 ──` and `✔ image`, and every assertion in BundleInstallSelection
            // and BundleImagePins looks for those bytes as text. Without this the reader takes the
            // test host's console code page — 437 on a Windows host whose test process has no
            // UTF-8 console, which is what `dotnet test` from Git Bash and from a build hands it —
            // and the same header reads back as `ΓöÇΓöÇ phase 5`, failing two daemon-free tests on
            // a machine where the script ran perfectly. Found on 2026-09-18, on the merge of the
            // cluster lanes, when the story class passed and the dry run did not.
