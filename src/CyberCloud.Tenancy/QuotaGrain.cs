...
public sealed class QuotaGrain(
    ...
    ///     ⚠
    ///     <b>
    ///         This is the mechanism behind docs/plan/06 § Quota's "expires on its own if the
    ///         operation grain dies".
    ///     </b>
    ///     It is deliberately longer than the index claim's five minutes:
    ...
    public async Task<Result<QuotaUsage>> ReturnAsync(QuotaMeter meter, decimal amount) {
        if (amount <= 0) {
            return Result<QuotaUsage>.Failure(
                ErrorCode.InvalidRequestBody,
                $"A return must be positive; {amount} is not."
            );
        }
    ...
    public async Task<Result<QuotaUsage>> SetLimitAsync(QuotaMeter meter, decimal limit) {
        ...
        if (limit < 0) {
            return Result<QuotaUsage>.Failure(ErrorCode.InvalidRequestBody, $"A limit cannot be negative; {limit} is.");
        }
    ...
    decimal Committed(QuotaMeter meter) => state.State.Committed.TryGetValue(meter, out var value) ? value : 0m;

    QuotaUsage UsageOf(QuotaMeter meter) =>
        new() {
            Meter = meter,
