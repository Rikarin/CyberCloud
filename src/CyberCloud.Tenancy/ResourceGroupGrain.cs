    ...
    : Grain, IResourceGroupGrain
{
    ...
    public async Task<Result<ResourceGroupDescriptor>> CreateAsync(Guid tenantId, string region)
    {
        ...
        state.State.Descriptor = new ResourceGroupDescriptor
        {
            Name = name,
    ...
    public Task<Result<ResourceGroupDescriptor>> GetAsync() =>
        Task.FromResult(state.State.Descriptor is { } descriptor
            ...
            : Result<ResourceGroupDescriptor>.Failure(TenancyGrainKeys.NotCreated(
                ErrorCode.ResourceGroupNotFound, "Resource group", name)));
    ...
    public async Task<Result> BeginCreateAsync(ResourceId address)
    {
        ...
        {
            return Result.Failure(TenancyGrainKeys.NotCreated(
                ErrorCode.ResourceGroupNotFound, "Resource group", name));
        }

        if (address.TenantId != tenantId || address.SubscriptionId != subscriptionId
        ...
        state.State.Members[address.Id] = new ResourceGroupMember
        {
            ResourceId = address.Id,
            CanonicalPath = address.CanonicalPath,
            State = ProvisioningState.Creating,
        };
    ...
    public async Task<Result> CompleteCreateAsync(Guid resourceId, ProvisioningState terminal)
    {
        ...
        {
            return Result.Failure(TenancyGrainKeys.NotCreated(
                ErrorCode.ResourceNotFound, "Resource", resourceId.ToString("D")));
        }
    ...
    public async Task<Result> FailDeleteAsync(Guid resourceId, string failure)
    {
        ...
        state.State.Members[resourceId] = member with
        {
            ...
            LastFailure = failure,
            TeardownAttempts = member.TeardownAttempts + 1,
        };
    ...
    public Task<Result<IReadOnlyList<ResourceGroupMember>>> ListAsync() =>
        Task.FromResult(Result<IReadOnlyList<ResourceGroupMember>>.Success(
