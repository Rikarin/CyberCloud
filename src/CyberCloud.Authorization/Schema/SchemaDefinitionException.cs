...
/// <remarks>
///     <para>
///         ⚠ <b>An exception rather than a <c>Result</c>, and that is the repository's own rule
///         applied rather than an exception to it.</b> docs/plan/00 § Non-negotiables reserves
///         <c>Result</c> for domain outcomes and exceptions for bugs. A schema is <b>compiled
///         C#</b> (docs/plan/07 § The model) — nothing outside this process can produce one, so a
...
public sealed class SchemaDefinitionException : Exception
{
    ...
    public SchemaDefinitionException(string message, IEnumerable<string> problems)
        : base(Compose(message, problems)) => Problems = [.. problems];
