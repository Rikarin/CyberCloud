...
public abstract class RelationExpression
{
    ...
    public static RelationExpression operator |(RelationExpression left, RelationExpression right) =>
        Or(left, right);
...
{
    internal RelationRefExpression(string relation) => Relation = relation;
...
/// </summary>
/// <remarks>
///     docs/plan/07 § The model: <i>"It is the whole of hierarchical inheritance and it is why a
///     role assignment at a subscription grants on every resource group in it without any tuple
///     being written per resource."</i> <c>RoleAssignmentViewTests</c> asserts exactly that
...
{
    internal UnionExpression(ImmutableArray<RelationExpression> operands) => Operands = operands;
...
{
    internal IntersectionExpression(ImmutableArray<RelationExpression> operands) => Operands = operands;
...
/// <summary>
///     An exclusion — <c>!a</c>. ⚠ <b>The only non-monotone node, and the one the schema builder
///     spends most of its rules on.</b>
/// </summary>
/// <remarks>
///     docs/plan/07 § Caching across requests: <i>"A permission of the form <c>A &amp; !B</c> is not
///     monotone: adding a tuple can remove access. Any cache that assumes 'more tuples can only
///     grant more' is wrong in the presence of <c>!</c>."</i> The restriction that makes the cache
...
{
    internal ExclusionExpression(RelationExpression operand) => Operand = operand;
