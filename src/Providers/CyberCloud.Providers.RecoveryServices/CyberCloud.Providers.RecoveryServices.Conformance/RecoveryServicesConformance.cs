...
///         ⚠
///         <b>
///             THE FIRST CASE WITH A <see cref="Companions" /> ENTRY, AND WHY ONE OBJECT PLANTED
///             THROUGH <see cref="ProviderConformanceCase.OperatorWritten" /> WOULD NOT HAVE DONE.
///         </b>
...
///         ⚠
///         <b>
///             <see cref="ProviderConformanceCase.Objects" /> is the ScheduledBackup and nothing
///             else.
///         </b> The Backup objects a schedule produces are the operator's — the controller
...
public sealed class RecoveryVaultCase : IProviderCaseSource {
    ...
    ///         ⚠
    ///         <b>
    ///             The fake and the harness's CRD stub admit that body; CloudNativePG's real definition
    ///             does not.
    ///         </b> An empty <c>destinationPath</c> is refused, and one that is filled in is
    ...
    public static ProviderConformanceCase ProviderCase { get; } =
        new() {
            ...
            CreateProvider = static () => new RecoveryServicesProvider(),
            ReconcilerType = typeof(RecoveryVaultReconciler),
            CreateReconciler = static clock => new RecoveryVaultReconciler(clock),
            Type = RecoveryVaults.Type,
            ApiVersion = RecoveryVaults.V2026,
            Body = static cluster => RecoveryVaults.Body(cluster, [ProtectedServer.Address().Path]),
            ...
            ChangedBody = static cluster => RecoveryVaults.Body(
                cluster,
                [ProtectedServer.Address().Path],
                "30 3 * * *"
            ),
            // Drops the required `/properties/policy/schedule`.
            InvalidBody = static cluster => WithoutSchedule(
                RecoveryVaults.Body(cluster, [ProtectedServer.Address().Path])
            ),
            InvalidBodyTarget = "/properties/policy/schedule",
            ActionName = RecoveryVaults.ListRecoveryPointsAction,
            Objects = static (id, ns) => [RecoveryVaults.ScheduledBackupRef(ns, id.Name, ProtectedServer.Name)],
...
public sealed class RecoveryVaultClusterBackedConformance() :
    ClusterBackedConformanceTests(RecoveryVaultCase.ProviderCase);
...
public sealed class RecoveryVaultSuiteShapeTests {
    ...
    public void ThePlantedRecoveryPointIsLabelledTheWayTheOperatorLabelsAndOwnedByTheSchedule() {
        ...
        RecoveryVaults.BackupParentOf(json)
            .ShouldBe(schedule, "the operator's own label is the join listRecoveryPoints selects on");
        ...
        owner["uid"]!.GetValue<string>()
            .ShouldBeEmpty("an empty uid is what tells the harness to fill in the applied object's");
        ...
        labels.Select(static x => x.Key)
            .ShouldNotContain(x => x.StartsWith("cybercloud.io/", StringComparison.Ordinal));
    }
