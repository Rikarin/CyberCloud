...
public sealed class RecoveryVaultActionHandlerTests {
    ...
    public async Task ListRecoveryPointsAnswersEveryItemsPointsNewestFirstWithTheCompletedCount() {
        ...
        connection.Plant(RecoveryVaults.BackupRef(ns, main + "-1"), RecoveryVaults.OperatorBackupJson(ns, main, "main", main + "-1", "completed", Now.AddDays(-2), Now.AddDays(-2).AddMinutes(3)));
        connection.Plant(RecoveryVaults.BackupRef(ns, main + "-2"), RecoveryVaults.OperatorBackupJson(ns, main, "main", main + "-2", "failed", Now.AddDays(-1), Now.AddDays(-1).AddMinutes(1), error: "invalid destination: no bucket"));
        connection.Plant(RecoveryVaults.BackupRef(ns, reports + "-1"), RecoveryVaults.OperatorBackupJson(ns, reports, "reports", reports + "-1", "completed", Now.AddHours(-3), Now.AddHours(-3).AddMinutes(2)));
        // Somebody else's, under a schedule this vault does not own.
        connection.Plant(RecoveryVaults.BackupRef(ns, "weekly-main-1"), RecoveryVaults.OperatorBackupJson(ns, "weekly-main", "main", "weekly-main-1", "completed", Now, Now));

        var answer = await new RecoveryVaultListRecoveryPointsHandler().InvokeAsync(Context(vault, connection, ns, RecoveryVaults.ListRecoveryPointsAction, "{}"), TestContext.Current.CancellationToken);
        ...
        var lines = body["recoveryPoints"]!.AsArray().Select(x => x!.GetValue<string>()).ToList();
        ...
        lines[1].ShouldEndWith(": invalid destination: no bucket", customMessage: "the operator's error text is the line a tenant needs to read");
    ...
    public async Task ListRecoveryPointsFailsRatherThanAnsweringZeroWhenTheClusterCannotBeListed() {
        ...
        var answer = await new RecoveryVaultListRecoveryPointsHandler()
            .InvokeAsync(Context(vault, new RecordingConnection { RefuseListing = true }, Ids.Namespace(vault), RecoveryVaults.ListRecoveryPointsAction, "{}"), TestContext.Current.CancellationToken);
    ...
    public async Task RecoverBootstrapsANewClusterFromACompletedPointSizedLikeTheSource() {
        ...
        connection.Plant(RecoveryVaults.BackupRef(ns, point), RecoveryVaults.OperatorBackupJson(ns, schedule, "main", point, "completed", Now.AddDays(-1), Now.AddDays(-1).AddMinutes(4)));
        connection.Plant(
        ...
        var answer = await new RecoveryVaultRecoverHandler().InvokeAsync(Context(vault, connection, ns, RecoveryVaults.RecoverAction, request), TestContext.Current.CancellationToken);
        ...
        var restored = connection.Applied.Single(x => x.Target.Kind.Kind == "Cluster");
        ...
        spec["storage"]!["size"]!.GetValue<string>().ShouldBe("50Gi", "a recovery needs a volume at least as large as the one it came from");
        ...
        spec.ContainsKey("backup").ShouldBeFalse("a restored cluster that archived into the source's store under the source's name would overwrite its WAL");
    ...
    public async Task RecoverProceedsWithDefaultsWhenTheSourceClusterIsGone() {
        ...
        connection.Plant(RecoveryVaults.BackupRef(ns, point), RecoveryVaults.OperatorBackupJson(ns, schedule, "main", point, "completed", Now.AddDays(-1), Now.AddDays(-1)));

        var request = new JsonObject { ["recoveryPoint"] = point, ["targetName"] = "main-again" }.ToJsonString();
        var answer = await new RecoveryVaultRecoverHandler().InvokeAsync(Context(vault, connection, ns, RecoveryVaults.RecoverAction, request), TestContext.Current.CancellationToken);

        answer.IsSuccess.ShouldBeTrue(answer.Error?.Message);
        var spec = JsonNode.Parse(connection.Applied.Single(x => x.Target.Kind.Kind == "Cluster").Body)!["spec"]!.AsObject();
    ...
    public async Task RecoverRefusesAPointThatIsNotThisVaultsWithTheSameAnswerAsAnAbsentOne() {
        ...
        theirs.Error.Message.Replace("weekly-main-1", "POINT", StringComparison.Ordinal)
            .ShouldBe(absent.Error.Message.Replace("nothing-here", "POINT", StringComparison.Ordinal), "the two answers differ, which tells a caller the other vault's point exists");
    ...
    public async Task RecoverRefusesAFailedPointAndAnExistingTargetByName() {
        ...
        connection.Plant(RecoveryVaults.BackupRef(ns, schedule + "-bad"), RecoveryVaults.OperatorBackupJson(ns, schedule, "main", schedule + "-bad", "failed", Now, Now, error: "invalid destination"));
        connection.Plant(RecoveryVaults.BackupRef(ns, schedule + "-good"), RecoveryVaults.OperatorBackupJson(ns, schedule, "main", schedule + "-good", "completed", Now, Now));
        connection.Plant(RecoveryVaults.ClusterRef(ns, "taken"), """{"kind":"Cluster","metadata":{"name":"taken"},"spec":{}}""");
    ...
    static async Task<(ResourceId Vault, RecordingConnection Connection, string Namespace)> ProtectedAsync(params string[] servers) {
        var vault = Ids.Vault("nightly");
        ...
        foreach (var server in servers) {
            view.Showing(Ids.Server(server), Ids.ServerBody(), Ids.Cluster, ProvisioningState.Succeeded, RecoveryVaults.ClusterRef(ns, server));
        }

        using var body = JsonDocument.Parse(RecoveryVaults.Body(Ids.Cluster, [.. servers.Select(x => Ids.Server(x).Path)]));

        (await new RecoveryVaultReconciler(new FixedClock()).ReconcileAsync(Ids.Context(connection, vault, body.RootElement, view), TestContext.Current.CancellationToken))
            .ShouldBe(ReconcileOutcome.Converged);

        return (vault, connection, ns);
    }

    static async Task<Result<string>> Recover(ResourceId vault, RecordingConnection connection, string ns, string point, string target) =>
        await new RecoveryVaultRecoverHandler().InvokeAsync(
            Context(vault, connection, ns, RecoveryVaults.RecoverAction, new JsonObject { ["recoveryPoint"] = point, ["targetName"] = target }.ToJsonString()),
            TestContext.Current.CancellationToken
        );

    static ActionContext Context(ResourceId vault, RecordingConnection connection, string ns, string action, string requestJson) {
        using var request = JsonDocument.Parse(requestJson);
        using var desired = JsonDocument.Parse(RecoveryVaults.Body(Ids.Cluster, []));

        return new(vault, RecoveryVaults.V2026, action, request.RootElement.Clone(), desired.RootElement.Clone(), ns, connection, new UnavailableSecretResolver());
    }
}
