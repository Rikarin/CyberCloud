...
/// <summary>
///     Every property name of the object-storage type, spelled the same in all three places a caller
///     meets it.
/// </summary>
/// <remarks>
///     <para>
...
///         docs/plan/15 § The three kinds, <c>charts/managed/seaweedfs/Chart.yaml</c>'s
...
///     </para>
/// </remarks>
public sealed class StorageOpenApiCasingTests {
    /// <summary>The spelling, written out. ⚠ <b>A literal, and it has to be.</b></summary>
    const string QualifiedType = "CyberCloud.Storage/accounts";
    ...
    public void EveryDeclaredPropertyNameReachesTheOpenApiDocumentWithItsExactCasing() {
        ...
        foreach (var property in StorageAccounts.Schema2026.Properties) {
            names.ShouldContain(
                property.Name,
    ...
    public void TheCamelCasedPropertyNamesAreSpelledOutHereRatherThanDerived() {
        ...
        foreach (var expected in new[] { "volumeServers", "accessKeyId", "secretAccessKey" }) {
    ...
    [Fact]
    public void TheApiSpellsReplicationAndTheCustomResourceSpellsDefaultReplication() {
        // ⚠ TWO VOCABULARIES, FOUR LINES APART IN THE SAME PROVIDER, AND ONLY ONE OF THEM IS THE API.
        // `defaultReplication` is SeaweedFS' own field name on spec.master and is written by
        // StorageAccounts.SeaweedJson; `replication` is the property a tenant sets. A document
        // carrying the operator's spelling would be a body every generated client sends to a key the
        // write path refuses as unknown — and the mistake reads as correct, because the value it
        // carries genuinely is the default replication.
        var names = new HashSet<string>(StringComparer.Ordinal);
        Collect(Document(), names);

        names.ShouldContain("replication");
        names.ShouldNotContain(
            "defaultReplication",
            "the operator's own field name reached the API document. It is the SeaweedFS CR's "
            + "spelling, not this platform's, and the two must not be the same string in the same "
            + "surface."
        );
    }
    ...
    public void TheActionIsRoutedUnderItsDeclaredCasingRatherThanLowerCased() {
        Paths().ShouldContain(
            x => x.EndsWith("/" + StorageAccounts.ListKeysAction, StringComparison.Ordinal),
            $"no path ends with '/{StorageAccounts.ListKeysAction}'"
        );
    ...
    public void TheKubeLabelValueIsTheLowerCasedTypeAndIsLegalLabelSyntax() {
        ...
        // by `_`. charts/managed/seaweedfs/conformance.yaml pins the same string independently.
