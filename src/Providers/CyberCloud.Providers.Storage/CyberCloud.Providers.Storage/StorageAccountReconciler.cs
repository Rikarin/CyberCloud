...
/// <remarks>
///     <para>
///         ⚠ <b>ONE OBJECT, WHICH MAKES THIS THE SHORTEST RECONCILER SINCE THE SAMPLE AND THE LARGEST
///         WORKLOAD IN THE CATALOGUE.</b> A <c>Seaweed</c> expands into master, volume, filer and S3
///         Deployments/StatefulSets, their Services, and four <c>ServiceMonitor</c>s when monitoring
///         is on. None of those is applied here, and writing any of them would be this provider
///         competing with the controller that owns them — the rule <c>charts/managed/valkey</c>
///         states. The consequence worth having written down: object <i>count</i> is not a measure of
///         a service's size, which the NATS row's five-objects-for-no-operator and this row's
///         one-object-for-everything now bracket from both ends.
///     </para>
///     <para>
...
///             ⚠ <b>And on this type it will keep coming back forever until piece 5 lands</b>, because
///             the S3 gateway mounts a <c>Secret</c> nothing writes — see
///             <see cref="StorageAccounts.ConfigSecretName" />. That is the intended shape: a resource
///             that visibly has not finished, rather than one that quietly came up open.
///         </item>
///         <item>
...
public sealed class StorageAccountReconciler(IClock clock) : IResourceReconciler {
    ...
    public async Task<ReconcileOutcome> ReconcileAsync(
        ...
        context.Log.Report("applying", $"applying the Seaweed of '{name}' to {context.Namespace}", 20);
        ...
        switch (outcome.Result) {
            ...
            case ApplyResult.Conflict:
                ...
                return ReconcileOutcome.InProgress(
                    ...
                    ?? "another field manager owns part of the Seaweed and it was not overwritten",
                    TimeSpan.FromSeconds(30)
                );
        }

        // ── Clause 4. Everything above this line is a claim; this is the reading. ───────────────
        var target = StorageAccounts.SeaweedRef(context.Namespace, name);
        ...
        context.Log.Report("ready", $"the Seaweed of '{name}' reads back as desired", 100);
    ...
    public async Task<ReconcileOutcome> DeleteAsync(
        ...
        context.Log.Report("deleting", $"deleting the Seaweed of '{name}'");
        ...
        var deleted = await KubeCommand.For(cluster)
            ...
            // collector removing every dependent — and on this type the dependents are four workloads,
            // their pods and every volume-server PVC — while a converge loop with a bounded PASS
            // budget runs out of passes waiting for a controller it does not drive. The read-back
            // below is what makes Background safe: this returns Converged when the object is GONE,
            // not when the delete was issued.
        ...
        context.Log.Report("deleted", $"the Seaweed of '{name}' is gone", 100);
        return ReconcileOutcome.Converged;
    }
