...
public sealed class NetworkReconcilerTests {
    ...
    public void TheStructuralCheckStillMissesAReadonlyMutableCache() {
        // ⚠ THE BLIND SPOT, PINNED AGAINST A COUNTER-EXAMPLE THAT SHOULD FAIL AND DOES NOT — SIXTH
        // SIGHTING, IN THE SIXTH FAMILY. CheckNoHiddenState skips a `readonly` field, and a
        // `readonly` Dictionary is mutable forever, which is exactly the shape a per-tenant cache
        // takes when somebody adds one for performance. Only the cross-tenant test below catches it.
        //
...
///     somebody adds one for performance.
/// </remarks>
