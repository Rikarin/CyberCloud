...
///     reasons, as <c>CyberCloud.Communication.CommunicationGrainDecoder</c>.
/// </summary>
/// <remarks>
///     <para>
///         A grain's key is its identity, and the evaluator begins by asking what its key says.
...
///     </para>
/// </remarks>
static class MonitorGrainDecoder {
    ...
    public static Guid ResourceOf(IAddressable grain) {
        ...
        if (key.Kind != GrainKeyKind.Resource) {
            throw new InvalidOperationException(
                ...
                + $"which is a {key.Kind} key. Address it through MonitorAlertRules.EvaluatorIdFor — an "
                + "evaluator reached through the wrong key shape would evaluate another workspace's "
                + "rules against this one's store."
            );
        }
