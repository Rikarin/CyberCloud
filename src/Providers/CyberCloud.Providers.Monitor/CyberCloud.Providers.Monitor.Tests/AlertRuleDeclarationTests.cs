...
public sealed class AlertRuleDeclarationTests {
    ...
    static string Body(string? service = null, params string[] recipients) =>
        MonitorAlertRules.Body(
            service ?? ServicePath(),
            recipients.Length == 0 ? ["oncall@example.com"] : [.. recipients]
        );
    ...
    public void TheTypeIsAClusterlessChartlessChildOfAClusterBackedParent() {
        ...
        rules.RequiresCluster.ShouldBeFalse(
            "an alert rule reads no cluster connection; declaring one makes the driver refuse every pass"
        );
    ...
    public void ListInstancesIsSynchronousWithAHandlerThatServesTheTypeAndTheAction() {
        ...
        var action = registration.Actions.Single(static x => x.Name == MonitorAlertRules.ListInstancesAction);
    ...
    public void TheBodyHelperSatisfiesTheSchemaAndEveryPointerIsCamelCase() {
        ...
        MonitorAlertRules.Schema2026.Properties.Single(static x => x.JsonPointer == "/properties/actionGroup/service")
    ...
    public void TheThreeMandatoryLimitsAreInTheSchemaOrTheGrainAndNotInProse() {
        ...
        var lookback = MonitorAlertRules.Schema2026.Properties.Single(static x => x.JsonPointer
        ...
        var query = MonitorAlertRules.Schema2026.Properties.Single(static x => x.JsonPointer
            == "/properties/condition/query"
        );
        query.MaxLength.ShouldBe(MonitorAlertRules.MaxQueryLength);

        var interval = MonitorAlertRules.Schema2026.Properties.Single(static x => x.JsonPointer
            == "/properties/evaluation/intervalSeconds"
        );
        interval.Minimum.ShouldBe(
            IAlertEvaluatorGrain.Tick.TotalSeconds,
            "a rule cannot ask to be evaluated more often than the reminder ticks"
        );

        IAlertEvaluatorGrain.MaxRules.ShouldBeGreaterThan(0);
        IAlertEvaluatorGrain.QueryTimeout.ShouldBeLessThan(
            IAlertEvaluatorGrain.Tick,
            "a query that outlives the tick queues the next tick behind it"
        );
        ...
        responseTimeout.ShouldBe(
            TimeSpan.FromSeconds(30),
            "the arithmetic in IAlertEvaluatorGrain's remarks was done against 30 s"
        );
        ...
        IAlertEvaluatorGrain.PassBudget.ShouldBeLessThan(
            IAlertEvaluatorGrain.Tick,
            "a pass that outlives the tick queues the next tick behind it"
        );

        // And the reads are what a reconcile pass and an action call first, so they interleave.
        foreach (var name in new[] {
                     nameof(IAlertEvaluatorGrain.GetRuleAsync), nameof(IAlertEvaluatorGrain.ListRulesAsync),
                     nameof(IAlertEvaluatorGrain.IsArmedAsync)
                 }) {
    ...
    public void AnActionGroupNamingAnotherTenantsServiceIsRefusedAtThePointer() {
        var refused = Spec(Rule(), Body(ServicePath(Guid.Parse("22222222-2222-4222-8222-222222222222"))));
    ...
    public void AnActionGroupWithNoUsableRecipientIsRefusedAtThePointer() {
        ...
        var tooMany = Spec(
            ...
            Body(
                null,
                [.. Enumerable.Range(0, MonitorAlertRules.MaxRecipients + 1).Select(static i => $"r{i}@example.com")]
            )
        );
    ...
    public void AnIntervalThatIsNotAMultipleOfTheTickIsRefusedAtThePointer() {
        ...
        MonitorAlertRules.Schema2026.Validate(valid.RootElement)
            .IsSuccess.ShouldBeTrue("the schema can refuse it after all, so ToSpec's refusal is redundant");
        ...
        Spec(
            Rule(),
            MonitorAlertRules.Body(ServicePath(), ["oncall@example.com"], intervalSeconds: 300)
    ...
    public void MatchesIsTheSpecComparisonAndSeesEveryMemberThatMatters() {
        ...
        MonitorAlertRules.Matches(held, id, same.RootElement)
            .ShouldBeTrue("state and history are not part of the comparison");

        using var threshold = JsonDocument.Parse(MonitorAlertRules.Body(ServicePath(), ["oncall@example.com"], 6));
        ...
        using var disabled = JsonDocument.Parse(
            MonitorAlertRules.Body(ServicePath(), ["oncall@example.com"], enabled: false)
        );
    ...
    public void EveryVocabularyWordRoundTripsThroughItsSpelling() {
        ...
        foreach (var kind in Enum.GetValues<ChannelKind>().Where(static x => x != ChannelKind.Unknown)) {
    ...
    public void TheInstanceLineAndTheSummaryReadAsOneSentenceEach() {
        ...
        MonitorAlertRules.Summary(spec, 7.25, at, true)
            .ShouldBe(
                "[warning] errors-high on workspace prod FIRING at 2026-09-15T14:22:09.0000000+00:00: max(rate(http_requests_errors_total[5m])) = 7.25 > 5"
            );
        ...
        MonitorAlertRules.InstanceLine(instance)
            .ShouldBe(
                "firing critical fired 2026-09-15T14:22:09.0000000+00:00 resolved - value 7.25: s | fire a@example.com: sent (Dispatched)"
            );

        var json = JsonNode.Parse(
            MonitorAlertRules.InstancesJson(
                new() { Spec = spec, State = AlertRuleState.Firing, Instances = [instance] }
            )
        )!;
    ...
    public void TheEvaluatorIdIsAVersionEightGuidAndNeverEmpty() {
        ...
        id.ToString("N")[12]
            .ShouldBe(
                '8',
                "RFC 9562's custom version, which is what a SHA-256 name-based id honestly is"
            );
    }
