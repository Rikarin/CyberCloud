...
namespace CyberCloud.Providers.Compute.KubeVirtConformance;
...
///     with the class it accompanies, because the trap is per assembly:
///     <c>test/CyberCloud.Bundle.Cluster.Conformance</c> keeps its own three companions. What it
///     asserts that the cluster class cannot see is the ORDER: three
...
///         30-minute timeout — with the bundle suite at 3 m 49 s of it. Eight minutes more would
///         have timed the job out on every PR. #25 measured the same chain and split it off the PR
///         as <c>--test-lane Cluster</c> (<c>build/Build.Test.cs § TestLane</c>): the
///         <c>*.Cluster.Conformance</c> suites run in main.yml's <c>test-cluster</c> job on every
///         merge and in nightly.yml's full <c>Test</c> run, never on a PR. So the class moved to a
///         project with that suffix, <c>CyberCloud.Providers.Compute.KubeVirt.Cluster.Conformance</c>,
///         named the way <c>CyberCloud.Providers.RecoveryServices.Cnpg.Cluster.Conformance</c> is for
///         the operator it installs; the bundle suite's three classes stay at under five minutes and
///         ride the same lane.
///     </para>
///     <para>
...
///         inside eight minutes of its own work, and a run of this assembly alone reported 9 m 40 s
///         with the wait for <c>ClusterSlot</c> behind another suite included.
///     </para>
/// </remarks>
