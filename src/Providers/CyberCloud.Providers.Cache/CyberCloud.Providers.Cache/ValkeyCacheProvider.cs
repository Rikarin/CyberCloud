...
///     </para>
///     <para>
///         ⚠ <b>Seven of docs/plan/12 § The pattern, once's eight pieces are here or next door, and
///         the one that is not is named rather than implied.</b> The chart (1), its generated schema
...
///     </para>
///     <para>
///         ⚠ <b>Piece 5 is this type's exception, and it is the operator's doing rather than a
///         choice.</b> For every other data provider the credential is generated by the operator at
///         bootstrap and the handler READS it, because anything minted afterwards is a password the
///         server never accepted while everything reports success. <b>spotahome generates
///         nothing</b> — <c>service/k8s/util.go</c>'s <c>GetRedisPassword</c> reads a <c>Secret</c> and
