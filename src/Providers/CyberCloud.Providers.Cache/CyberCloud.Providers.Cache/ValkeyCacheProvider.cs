...
///         <i>
///             "Valkey — <c>CyberCloud.Cache/redis</c> · M1 · 1.0
///             EM"
///         </i>, on <c>spotahome/redis-operator</c> (ADR-010 clause 1). ADR-011 rejects Redis ≥ 7.4
...
///         ⚠
///         <b>
///             Seven of docs/plan/12 § The pattern, once's eight pieces are here or next door, and
///             the one that is not is named rather than implied.
///         </b> The chart (1), its generated schema
///         (2), this registration (3), the reconciler (4), credential provisioning (5) and the
///         conformance manifest (8) exist; monitoring (6) is the operator's own exporter, switched by
///         one annotated boolean rendered into <c>spec.redis.exporter.enabled</c> and
///         <c>spec.sentinel.exporter.enabled</c>, which is the correction docs/plan/12 records — ask
///         the operator for the scrape objects wherever it accepts the request.
///     </para>
///     <para>
///         ⚠
///         <b>
///             Piece 5 is this type's exception, and it is the operator's doing rather than a
///             choice.
///         </b> For every other data provider the credential is generated by the operator at
///         bootstrap and the handler READS it, because anything minted afterwards is a password the
///         server never accepted while everything reports success.
///         <b>
///             spotahome generates
///             nothing
///         </b> — <c>service/k8s/util.go</c>'s <c>GetRedisPassword</c> reads a <c>Secret</c> and
///         errors when it is not there — so <c>ValkeyCacheReconciler</c> mints into the tenant's vault
///         and renders the <c>Secret</c> from what the vault returned, under
///         <c>ISecretWriter</c>'s mint-once rule. Before that landed, every cache a tenant created
///         converged from the platform's point of view and sat <c>InProgress</c> on the cluster
///         forever.
///     </para>
///     <para>
///         ⚠
///         <b>
///             Piece 7, backup, is not declared at all, and that is this type's answer to the
///             question docs/plan/12 leaves open.
///         </b> That document asks whether piece 7 means
///         <i>
///             a policy
///             file some platform backup service reads
///         </i> or
///         <i>
///             the service is backed up by whatever
///             mechanism its operator already provides
///         </i>, and says the evidence points at the second.
///         This provider is the case the first reading has no answer for:
///         <b>
///             a cache is not backed
///             up
///         </b>. <c>persistence</c> is a restart-survival setting, not a backup, and offering a
...
///         delete is built and the manager honours the declaration —
///         <b>
///             and for a CACHE the answer is
///             still probably no.
///         </b> The window's whole argument is the data, and a Valkey cache's data is
...
public sealed class ValkeyCacheProvider : IResourceProvider {
    ...
    public void Describe(IProviderBuilder builder) {
        ...
        builder
            ...
            // `longRunning: true` with a handler is refused by ProviderBuilder.Action — the two
            // together are why a listKeys cannot be declared as an operation: it would answer 202 and
            // hand back nothing.
            .Action(
