...
public sealed class ReconcilerTests(CommunicationTestCluster cluster) {
    ...
    public async Task ASecondResourceForTheSameKindIsRefusedByNameAndWritesNothing() {
        ...
        var first = await cluster.ConvergedEmailChannelAsync("one-owner", maxMessagesPerDay: 100);

        var second = CommunicationTestCluster.Child(CommunicationChannels.Type, "one-owner", "email-again");
        var refused = await cluster.ReconcileAsync(second, CommunicationChannels.Body(kind: "email", provider: "in-memory", maxMessagesPerDay: 5));
        ...
        var held = await cluster.Plane.GetChannelAsync(CommunicationTestCluster.Tenant, CommunicationServices.ServiceIdOf(first), ChannelKind.Email, CommunicationTestCluster.Ct);
        ...
        var deleted = await cluster.DeleteAsync(second, CommunicationChannels.Body(kind: "email", provider: "in-memory"));
        deleted.IsConverged.ShouldBeTrue();
        (await cluster.Plane.GetChannelAsync(CommunicationTestCluster.Tenant, CommunicationServices.ServiceIdOf(first), ChannelKind.Email, CommunicationTestCluster.Ct))
            .IsSuccess.ShouldBeTrue("deleting a channel resource that did not own the kind removed the owner's configuration");
    }
    ...
    public async Task ATenantAccountWithoutHandlesIsRefusedRatherThanBilledAtThePlatformRate() {
        ...
        var refused = await cluster.ReconcileAsync(channel, CommunicationChannels.Body(kind: "sms", provider: "in-memory", account: "tenant"));
        ...
        var converged = await cluster.ReconcileAsync(
            channel,
            CommunicationChannels.Body(kind: "sms", provider: "in-memory", account: "tenant", accountRef: "tenants/x/twilio#sid", authRef: "tenants/x/twilio#token@3")
        );

        converged.IsConverged.ShouldBeTrue(converged.ToString());

        var held = (await cluster.Plane.GetChannelAsync(CommunicationTestCluster.Tenant, CommunicationServices.ServiceIdOf(channel), ChannelKind.Sms, CommunicationTestCluster.Ct)).GetValueOrThrow();
        ...
        held.Credentials.AuthRef.ShouldBe(new CarrierSecretRef { Path = "tenants/x/twilio", Field = "token", Version = "3" });
    }
    ...
    public async Task AChildAheadOfItsServiceWaitsRatherThanFailing() {
        ...
        var waiting = await cluster.ReconcileAsync(channel, CommunicationChannels.Body(kind: "email", provider: "in-memory"));
        ...
        (await cluster.ReconcileAsync(channel, CommunicationChannels.Body(kind: "email", provider: "in-memory"))).IsConverged.ShouldBeTrue();
    ...
    public async Task ATemplateAppendsAVersionOnlyWhenTheBodyChangesAndKeepsItsHistoryAcrossRecreation() {
        ...
        (await cluster.Plane.ListTemplateVersionsAsync(CommunicationTestCluster.Tenant, templateId, CommunicationTestCluster.Ct))
            .GetValueOrThrow().Length.ShouldBe(1, "three identical passes appended more than one version");
        ...
        var versions = (await cluster.Plane.ListTemplateVersionsAsync(CommunicationTestCluster.Tenant, templateId, CommunicationTestCluster.Ct)).GetValueOrThrow();
        ...
        (await cluster.Plane.ResolveTemplateAsync(CommunicationTestCluster.Tenant, serviceId, "otp", CommunicationTestCluster.Ct)).IsFailure.ShouldBeTrue();
        ...
        (await cluster.Plane.ListTemplateVersionsAsync(CommunicationTestCluster.Tenant, CommunicationTemplates.TemplateIdOf(recreated), CommunicationTestCluster.Ct))
            .GetValueOrThrow().Length.ShouldBe(3, "the recreated template did not continue its history");
    }
    ...
    public async Task ATemplateRecreatedForAnotherChannelIsRefusedRatherThanSilentlyKeptOnTheOld() {
        ...
        (await cluster.ReconcileAsync(template, CommunicationTemplates.Body(channel: "email"))).IsConverged.ShouldBeTrue();
        (await cluster.DeleteAsync(template, CommunicationTemplates.Body(channel: "email"))).IsConverged.ShouldBeTrue();

        var recreated = CommunicationTestCluster.Child(CommunicationTemplates.Type, "channel-locked", "greeting");
        var refused = await cluster.ReconcileAsync(recreated, CommunicationTemplates.Body(channel: "sms"));
    ...
    public async Task TheServicesDefaultLocaleReachesASendThatNamesNone() {
        ...
        (await cluster.ReconcileAsync(service, CommunicationServices.Body(defaultLocale: "cs"))).IsConverged.ShouldBeTrue();
        ...
        (await cluster.ReconcileAsync(czech, CommunicationTemplates.Body(body: "Váš kód je {code}.", locale: "cs"))).IsConverged.ShouldBeTrue();

        var request = CommunicationTestCluster.Send(service, "someone@example.com", "loc-1") with { TemplateName = "otp-cs", Arguments = [new() { Name = "code", Value = "1" }], Body = "" };
        var sent = await cluster.Sender.SendAsync(CommunicationTestCluster.Tenant, request, CommunicationTestCluster.Ct);
    ...
    public async Task RetiringAServiceStopsSendsAndKeepsItsSuppressionList() {
        ...
        (await cluster.ReconcileAsync(block, CommunicationSuppressions.Body(destination: "kept@example.com"))).IsConverged.ShouldBeTrue();
        ...
        var refused = await cluster.Sender.SendAsync(CommunicationTestCluster.Tenant, CommunicationTestCluster.Send(service, "anyone@example.com", "retired-1"), CommunicationTestCluster.Ct);
        ...
        var check = await cluster.Plane.CheckSuppressionAsync(CommunicationTestCluster.Tenant, serviceId, ChannelKind.Email, "kept@example.com", CommunicationTestCluster.Ct);
    ...
    public void TheServiceIdIsDerivedFromTheAddressAndTheChildDerivesItsParents() {
        ...
        CommunicationServices.ServiceIdOf(service).ShouldNotBe(service.Id, "the grain id is the address's, not the resource manager's");
        ...
        CommunicationServices.ServiceIdOf(CommunicationTestCluster.Service("derive", CommunicationTestCluster.OtherTenant))
