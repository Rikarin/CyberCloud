...
public sealed class TemplateRenderTests {
    ...
    public async Task RendersSubjectAndBodyWithTheArgumentsSubstituted() {
        ...
        var rendered = await Handler.InvokeAsync(
            Context(desired.RootElement, request.RootElement),
        ...
        CommunicationTemplates.RenderResponse.Validate(body.RootElement)
            .IsSuccess.ShouldBeTrue("the render body does not match the published response shape");
    }

    [Fact]
    public async Task AMissingRequiredVariableIsRefusedNamingIt() {
        using var desired = JsonDocument.Parse(
            CommunicationTemplates.Body(body: "Hello {name}, your code is {code}.", variables: ["name", "code"])
        );
        ...
        refused.Error.Message.ShouldContain(
            "requires 'name'",
            customMessage: "the refusal names the missing variable, not the supplied one"
        );
    }
    ...
    public async Task AnUnsuppliedOptionalVariableIsLeftAsWrittenSoTheTenantSeesIt() {
        using var desired = JsonDocument.Parse(
            CommunicationTemplates.Body(
                body: "Code {code}; expires in {minutes}.",
                variables: ["code"],
                optionalVariables: ["minutes"]
            )
        );
    ...
    public void ParametersAreRequiredFirstThenOptionalAndNeverTwice() {
        using var desired = JsonDocument.Parse(
            CommunicationTemplates.Body(variables: ["code", "name"], optionalVariables: ["name", "minutes"])
        );

        var parameters = CommunicationTemplates.ParametersOf(desired.RootElement);

        parameters.Select(static x => (x.Name, x.Required))
            .ShouldBe(
                [("code", true), ("name", true), ("minutes", false)],
                "a name listed as both is required — the stricter reading"
            );
    }
    ...
    public void MatchesComparesContentAndChannelAndNeverTheVersionNumber() {
        using var desired =
            JsonDocument.Parse(CommunicationTemplates.Body("sms", "Hi {name}", "", variables: ["name"]));
        ...
        CommunicationTemplates.Matches(version with { Channel = ChannelKind.Email }, desired.RootElement)
            .ShouldBeFalse();
        CommunicationTemplates.Matches(
            version with { Bodies = [new() { Locale = "en", Body = "Hello {name}" }] },
            desired.RootElement
        )
            .ShouldBeFalse();
        CommunicationTemplates.Matches(
            version with { Parameters = ImmutableArray<TemplateParameter>.Empty },
            desired.RootElement
        )
            .ShouldBeFalse();
    }
