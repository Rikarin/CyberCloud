...
public sealed class CommunicationDeclarationTests {
    ...
    public void FourTypesAreDeclaredAndNoneRequiresACluster() {
        ...
        string[] declared = [.. registry.Types.Select(static x => x.Type.Type).Order(StringComparer.Ordinal)];
        ...
        declared.ShouldBe(
            ...
            "docs/plan/24 § Phase 3's Communication row: channels, templates, suppression — and the service they hang off"
        );
        ...
        foreach (var type in registry.Types) {
            ...
            type.RequiresCluster.ShouldBeFalse(
                $"'{type.Type}' declares RequiresCluster and converges grains, not objects"
            );
            ...
            type.ApiVersions.Select(static x => x.Version.Value).ShouldBe([CommunicationServices.V2026]);
        }
    }
        ...
        foreach (var type in Registry.Types) {
            foreach (var action in type.Actions) {
                action.LongRunning.ShouldBeFalse(
                    $"'{action.Name}' is declared long-running and this family has no long-running work"
                );
                ...
                var handler = (IResourceActionHandler)Activator.CreateInstance(
                    action.HandlerType!,
                    ArgumentsFor(action.HandlerType!)
                )!;
    ...
    public void TheServiceDeclaresTheFourActionsAndTheTemplateDeclaresRender() {
        ...
        service.Actions.Select(static x => x.Name)
        ...
        template.Actions.Select(static x => x.Name).ShouldBe([CommunicationTemplates.RenderAction]);
    ...
    public void TheFiveSpelledKindsAreTheFiveTheModuleHas() {
        ...
        var kinds = Enum.GetValues<ChannelKind>().Where(static x => x != ChannelKind.Unknown).ToArray();
        ...
        ChannelKinds.Parse("Email")
            .ShouldBe(ChannelKind.Unknown, "spellings are compared ordinally, as AllowedValues are");
    }

    [Fact]
    public void EveryStatusAndReasonTheResponsesSpellIsOneTheSchemasAllow() {
        foreach (var status in Enum.GetValues<MessageStatus>().Where(static x => x != MessageStatus.Unknown)) {
            CommunicationServices.StatusValues.ShouldContain(CommunicationServices.SpellStatus(status));
        }

        foreach (var reason in Enum.GetValues<SuppressionReason>().Where(static x => x != SuppressionReason.Unknown)) {
    ...
    public void TheSchemasDefaultsAreWhatTheReadersApply() {
        ...
        var channel = CommunicationChannels.ToConfiguration(
            CommunicationTestCluster.Service("x"),
            WithKind(empty.RootElement)
        )
            .GetValueOrThrow();
        ...
        channel.Limits.ShouldBe(
            ChannelLimits.None,
            "the schema publishes zero limits, which is the module's safe default"
        );
    ...
    static System.Text.Json.JsonElement WithKind(System.Text.Json.JsonElement _) {
        var document = System.Text.Json.JsonDocument.Parse(
            """{"location":"eu-central","properties":{"kind":"email"}}"""
        );
            ...
            .Select(object (x) => x.ParameterType == typeof(IMessageSender)
                ...
                : x.ParameterType == typeof(ICommunicationControlPlane) ? new RefusingPlane()
                    : throw new InvalidOperationException(
                        $"{handler.Name} takes a {x.ParameterType.Name}, which this test cannot supply"
                    )
            )
    ];
    ...
    sealed class RefusingPlane : ICommunicationControlPlane {
        public Task<Result<CommunicationService>> EnsureServiceAsync(
            Guid tenantId,
            Guid serviceId,
            string name,
        ...
        public Task<Result<MessageTemplateVersion>> EnsureTemplateAsync(
            Guid tenantId,
            Guid serviceId,
            Guid templateId,
            string name,
        ...
        public Task<Result<MessageTemplateVersion>> AddTemplateVersionAsync(
            Guid tenantId,
            Guid templateId,
            System.Collections.Immutable.ImmutableArray<TemplateParameter> parameters,
            System.Collections.Immutable.ImmutableArray<LocalizedBody> bodies,
            CancellationToken cancellationToken = default
        ) =>
            throw new NotSupportedException();
        ...
        public Task<
            Result<System.Collections.Immutable.ImmutableArray<MessageTemplateVersion>>> ListTemplateVersionsAsync(
            Guid tenantId,
            Guid templateId,
            CancellationToken cancellationToken = default
        ) =>
        ...
        public Task<Result<System.Collections.Immutable.ImmutableArray<SuppressionEntry>>> ListSuppressionsAsync(
            Guid tenantId,
            Guid serviceId,
            ChannelKind channel,
            CancellationToken cancellationToken = default
