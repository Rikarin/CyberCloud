...
public sealed class MembershipIndexPropertyTests {
        ...
        for (var seed = 0; seed < Graphs; seed++) {
            ...
            for (var step = 0; step < remaining.Count * 2 + 4; step++) {
                ...
                if (write && remaining.Count > 0) {
                    ...
                    if (Closure(graph.Schema, present).Members.Sum(x => x.Value.Count) < before.Members.Sum(x => x.Value.Count)) {
                        shrinkingDeletes++;
                    }
                } else {
                    continue;
            ...
            foreach (var subjectObject in store.Objects) {
                ...
                slice.Members.ShouldBeEmpty($"seed {seed}: {subjectObject} still has members after every tuple was deleted");
                slice.Usersets.ShouldBeEmpty($"seed {seed}: {subjectObject} is still in a userset after every tuple was deleted");
            }
        }
    ...
    public async Task ARebuiltSliceEqualsTheOneTheWritesProduced() {
        for (var seed = 0; seed < 500; seed++) {
            ...
            var maintainer = new MembershipIndexMaintainer(graph.Schema, new InMemoryRelationReader(tuples), reverse, reverse.Store);
            ...
            foreach (var subjectObject in Universe(tuples)) {
                ...
                Sets(change.AddMembers).ShouldBe(Sets(stored.Members), $"seed {seed}: a rebuild of {subjectObject}'s members disagrees with the writes");
                Sets(change.AddUsersets).ShouldBe(Sets(stored.Usersets), $"seed {seed}: a rebuild of {subjectObject}'s usersets disagrees with the writes");
            }
        }
    }
    ...
    static async Task ApplyWriteAsync(AuthorizationSchema schema, List<RelationTuple> present, InMemoryMembershipIndexStore store, RelationTuple tuple) {
    ...
    static async Task ApplyDeleteAsync(AuthorizationSchema schema, List<RelationTuple> present, InMemoryMembershipIndexStore store, RelationTuple tuple) {
    ...
    static void AssertIndexEquals(GeneratedGraph graph, InMemoryMembershipIndexStore store, List<RelationTuple> present, int step) {
        var expected = Closure(graph.Schema, present);
        var why = $"seed {graph.Seed}, step {step.ToString(CultureInfo.InvariantCulture)}, {present.Count} tuples present:"
            + Environment.NewLine
            + string.Join(Environment.NewLine, present.Select(x => "  " + x));
        ...
        foreach (var subjectObject in Universe(present).Concat(store.Objects).Distinct()) {
            ...
            var expectedMembers = expected.Members
                ...
                .ToDictionary(x => x.Key.Relation, x => x.Value, StringComparer.Ordinal);

            var expectedUsersets = expected.Usersets
                .Where(x => x.Key.Object == subjectObject && x.Value.Count > 0)
                .ToDictionary(x => x.Key.Relation, x => x.Value, StringComparer.Ordinal);

            Sets(slice.Members).ShouldBe(Sets(expectedMembers), $"{subjectObject}'s members differ from the recomputed closure — {why}");
            Sets(slice.Usersets).ShouldBe(Sets(expectedUsersets), $"{subjectObject}'s usersets differ from the recomputed closure — {why}");
        }
    }

    /// <summary>Both closures over a tuple set, by search and nothing cleverer.</summary>
    static (Dictionary<SubjectRef, HashSet<SubjectRef>> Members, Dictionary<SubjectRef, HashSet<SubjectRef>> Usersets) Closure(
        ...
        var startingUsersets = edges.Select(t => SubjectRef.Userset(t.Object.Type, t.Object.Id, t.Relation)).Distinct();
    ...
    static bool IsExpandable(AuthorizationSchema schema, SubjectRef subject) =>
        subject.IsUserset && schema.Member(subject.Type, subject.Relation) is { IsPermission: false, IsDirectOnly: true };

    static IEnumerable<ObjectRef> Universe(IEnumerable<RelationTuple> tuples) =>
        tuples.Select(t => t.Object).Concat(tuples.Select(t => t.Subject.Object)).Distinct();
    ...
    static string[] Sets<TValue>(IReadOnlyDictionary<string, TValue> lists)
        ...
        [
            .. lists.Where(x => x.Value.Any())
                .Select(x => x.Key + " => " + string.Join(", ", x.Value.Select(s => s.ToString()).Order(StringComparer.Ordinal)))
