...
///         ⚠
///         <b>
///             Against a scripted server, because the leak was in the shape of ClickHouse's
///             answer and not in any one query.
///         </b> The real server refuses a statement with its
...
public sealed class ResourceGraphQueryFailureTests {
    ...
    static readonly string[] NeverInTheBody = [
        "SELECT", "tenant_", "hasAny", "user:alice", "is_deleted", "DB::Exception", "clickhouse.internal", "http://",
        "Code: "
    ];
    ...
    [InlineData(
        0,
        "<html><body>502 Bad Gateway from the proxy in front of http://clickhouse.internal:8123/</body></html>"
    )]
    public async Task AStatementTheStoreRefusesIsA500WithOneSentenceAndTheDetailInTheLog(int code, string body) {
        ...
        var answered = await service.QueryAsync(
            new() {
                Query = "resources | where tags has 'prod'",
    ...
    [Theory]
    [InlineData(
        ClickHouseClient.TimeoutExceeded,
        "Code: 159. DB::Exception: Timeout exceeded: elapsed 10.2 seconds, maximum: 10: While executing MergeTreeSelect(pool: ReadPool, algorithm: Thread). (TIMEOUT_EXCEEDED)"
    )]
    [InlineData(
        ClickHouseClient.TooManyRows,
        "Code: 158. DB::Exception: Limit for rows (controlled by 'max_rows_to_read' setting) exceeded, max rows: 1.00 million, current rows: 1.05 million. (TOO_MANY_ROWS)"
    )]
    [InlineData(
        ClickHouseClient.TooManyRowsOrBytes,
    ...
    public async Task AQueryPastItsBudgetIsA400ThatNamesTheBudgetAndHowToNarrowIt(int code, string body) {
        var (service, log) = Service(
            new ScriptedHandler(
                code,
                body + " In scope SELECT name FROM tenant_11111111111141118111111111111111.resource_graph"
            )
        );
        ...
        log.Lines.ShouldContain(x => x.Contains("exceeded its budget", StringComparison.Ordinal)
            && x.Contains($"code {code}", StringComparison.Ordinal)
        );
    }
    ...
    public void TheExceptionCodeIsReadOffTheClientsOwnSpellingAndNotOffTheBody() {
        ClickHouseClient.ExceptionCode(
            new Error(
                ErrorCode.InternalError,
                "ClickHouse at http://x/ answered 500 Internal Server Error (X-ClickHouse-Exception-Code: 159): Code: 159. DB::Exception: …"
            )
        )
            .ShouldBe(159);
        ClickHouseClient.ExceptionCode(
            new Error(ErrorCode.InternalError, "ClickHouse at http://x/ could not be reached: refused")
        )
            .ShouldBe(0);
        // A body that names the header does not count; only the client's parenthesis does.
        ClickHouseClient.ExceptionCode(new Error(ErrorCode.InternalError, "X-ClickHouse-Exception-Code: 159"))
            .ShouldBe(0);
        ClickHouseClient.IsBudgetExceeded(
            new Error(
                ErrorCode.InternalError,
                "ClickHouse at http://x/ answered 500 Internal Server Error (X-ClickHouse-Exception-Code: 43): Code: 43."
            )
        )
            .ShouldBeFalse();
    }
    ...
    static (ResourceGraphQueryService Service, CapturingLogger Log) Service(HttpMessageHandler handler) {
        var options = new ResourceGraphOptions {
            ClickHouseEndpoint = "http://clickhouse.internal:8123/", AllowInsecureTransport = true
        };
        var client = new ClickHouseClient(new HttpClient(handler), options);
        var log = new CapturingLogger();

        return (new ResourceGraphQueryService(
            client,
            new ClickHouseResourceGraphStore(client),
            new OneCaller(),
            options,
            log
        ), log);
        ...
        protected override async Task<HttpResponseMessage> SendAsync(
            ...
            if (code != 0) {
                response.Headers.Add(
                    ClickHouseClient.ExceptionCodeHeader,
    ...
    sealed class ThrowingHandler : HttpMessageHandler {
        protected override Task<HttpResponseMessage> SendAsync(
            ...
            throw new HttpRequestException(
                "No connection could be made because the target machine actively refused it (clickhouse.internal:8123)"
            );
    }
