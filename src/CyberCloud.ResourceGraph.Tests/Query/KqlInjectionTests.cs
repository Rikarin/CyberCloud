...
/// </summary>
/// <remarks>
///     ⚠ <b>The assertion is on the SQL text and on the parameter list, never on ClickHouse's
///     behaviour.</b> A statement that ClickHouse happened to refuse would still have carried the
...
public sealed class KqlInjectionTests {
    ...
    public void AStringLiteralIsBoundAndNeverSpelledIntoTheStatement(string template) {
        var kql = template.Replace("{0}", Payload.Replace("'", "\\'", StringComparison.Ordinal), StringComparison.Ordinal);
        ...
        query.Parameters.ShouldContain(x => x.ClickHouseType == "String" && x.Value == Payload, "the payload is not bound as a string parameter");

        // … and the statement is still one SELECT over one table in the caller's database.
        query.Sql.Count(character => character == ';').ShouldBe(0);
        query.Sql.ShouldStartWith("SELECT ");
        query.Sql.Split(" FROM ").Count(fragment => fragment.Contains(".resource_graph", StringComparison.Ordinal)).ShouldBe(1);
    }
    ...
    public void AHasTermWithRegexMetacharactersIsEscapedInsideItsParameter() {
        ...
        // not everything.
        var translated = KqlTranslator.Translate("resources | where name has '.*(a|b)$'", KqlTranslationGoldenTests.Context());
        translated.IsSuccess.ShouldBeTrue(translated.Error?.Message);

        var pattern = translated.GetValueOrThrow().Parameters.Single(x => x.Name == "p0").Value;
    ...
    public void TheAccessSubjectsAreOneArrayParameterWithQuotesEscaped() {
        ...
        var translated = KqlTranslator.Translate("resources | count", new(KqlTranslationGoldenTests.Tenant, hostile, 10, 0));
        translated.IsSuccess.ShouldBeTrue(translated.Error?.Message);

        var access = translated.GetValueOrThrow().Parameters.Single(x => x.Name == KqlTranslator.AccessParameter);
    ...
    public void TheOnlyIdentifierInterpolatedIsTheTenantDatabaseAndItIsDerivedFromTheGuid() {
        ...
        var otherSql = KqlTranslator.Translate("resources | project name", new(other, KqlTranslationGoldenTests.Caller, 50, 0)).GetValueOrThrow().Sql;

        otherSql.ShouldBe(sql.Replace("tenant_11111111111141118111111111111111", "tenant_22222222222242228222222222222222", StringComparison.Ordinal));
    }
    ...
    public void ACallerWithNoSubjectsIsAProgrammingErrorAndNotAnEmptyFilter() {
        ...
        Should.Throw<ArgumentException>(() => KqlTranslator.Translate("resources", new(KqlTranslationGoldenTests.Tenant, [], 50, 0)));
    }
}
