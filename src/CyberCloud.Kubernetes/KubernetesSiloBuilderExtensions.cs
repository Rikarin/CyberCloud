...
public static class KubernetesSiloBuilderExtensions
{
    ...
    /// <remarks>
    ///     <para>
    ///         ⚠ <b><see cref="ClusterConnectionTenantFilter" /> is not optional and is registered
    ///         here so it cannot be forgotten.</b> It is what establishes
    ...
    public static ISiloBuilder AddCyberCloudKubernetes(this ISiloBuilder silo)
    {
        ...
        return silo.ConfigureServices(services =>
        {
            ...
            services.TryAddSingleton<IKubeApiClientFactory>(
                sp => new KubeApiClientFactory(sp.GetRequiredService<IClock>()));
