...
///             <b>state</b> and checks it on every call …
///             <b>
///                 This is the single place tenancy is enforced
///                 by code rather than by key
///             </b>
///             , and it is called out here so nobody has to discover
///             it."
///         </i>
///         <see cref="EnsureCallerMayReach" /> is that check. Every public method's first
...
public sealed class ClusterConnectionGrain : Grain, IClusterConnectionGrain {
    ...
    public ClusterConnectionGrain(
        [PersistentState("clusterConnection", StorageTiers.Durable)] IPersistentState<ClusterConnectionState> state,
    ...
    public override Task OnActivateAsync(CancellationToken cancellationToken) {
        ...
        health = new(clusterId, clock, options.HealthStalenessWindow);
    ...
    public async Task<Result> AttachAsync(ClusterConnectionDescriptor descriptor) {
        ...
        if (existing is null) {
            ...
            logger.LogInformation("Cluster {Cluster} attached: {Descriptor}.", clusterId, descriptor);
        } else {
    ...
    ///         ⚠
    ///         <b>
    ///             The refusal is <see cref="ErrorCode.ResourceNotFound" /> and not
    ///             <see cref="ErrorCode.AuthorizationFailed" />.
    ///         </b>
    ...
    void StartHealthTimer() {
        ...
        healthTimer = this.RegisterGrainTimer(
            ...
            new() { DueTime = TimeSpan.Zero, Period = options.PingInterval, Interleave = true }
        );
    }
}
