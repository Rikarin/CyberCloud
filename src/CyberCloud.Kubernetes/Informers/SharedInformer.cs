...
/// <remarks>
///     <para>
///         docs/plan/09 § Observing: <i>"Each connection grain runs shared informers for the GVKs its
...
///         O(kinds)."</i> "Shared" is the load-bearing word — <see cref="Subscribers" /> counts
///         holders and the watch is established once.
///     </para>
///     <para>
///         ⚠ <b>The failure this class is shaped around</b>, quoting docs/plan/09 § Observing:
///         <i>"The informer cache is in one silo's memory and is lost when that silo dies.
...
///         sounds — a 30-silo rolling deploy without staggering is a synchronized list storm."</i>
...
public sealed class SharedInformer
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (listed.TryGetError(out var error))
        {
            ...
            return Result<InformerEstablishment>.Success(new InformerEstablishment(
                Resumed: true,
                FellBackToFullList: true,
                StaggerDelay: stagger,
                ResourceVersion: afterFallback.ResourceVersion,
                ItemCount: afterFallback.ItemCount,
                Pages: afterFallback.Pages));
        }
    ...
    async Task<Result<ListOutcome>> ListAsync(string? resourceVersion, CancellationToken cancellationToken)
    {
        ...
        return Result<ListOutcome>.Success(new ListOutcome(cursor, items, pages));
    }
}
