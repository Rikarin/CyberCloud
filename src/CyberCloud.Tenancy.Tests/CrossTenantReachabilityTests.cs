...
public sealed class CrossTenantReachabilityTests(TenancyCluster cluster) {
    ...
    public async Task Route21_ThePlatformServiceEdgeIsOpenAndEveryOtherPlatformGrainIsNot() {
        ...
        CyberCloudGrainCallTenantSeparator.IsPlatformServiceEdge(
        ...
        CyberCloudGrainCallTenantSeparator.IsPlatformServiceEdge(
            CyberCloudGrainCallTenantSeparator.PlatformMessageGrainInterface,
            TenancyCluster.Id(A(21))
        )
            .ShouldBeFalse(
                "a message grain in ANOTHER tenant is another tenant's service — the spam route this must not open"
            );
        ...
        var thrown = await Should.ThrowAsync<UnauthorizedAccessException>(() => attacker.ReachTenantByRawKeyAsync(
                platformTenantKey
            )
        );
