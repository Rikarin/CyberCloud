...
/// <remarks>
///     <para>
///         <i>"An index grain is a hot spot if the indexed value is not high-cardinality.
...
///         key', and if the answer is 'small', it is not an index grain."</i>
///     </para>
///     <para>
...
public sealed class IndexGrainCardinalityTests(TenancyCluster cluster)
{
    ...
    public void TenThousandDistinctResourcePathsProduceTenThousandDistinctIndexKeys()
    {
        ...
        {
            keys.Add(GrainKeys.PathIndex(new ResourceId(
                tenant, subscription, "prod", type, "db-" + i, Guid.Empty)));
        }
    ...
    public async Task TwoResourcesOfTheSameTypeInOneGroupAreTwoIndexActivations()
    {
        ...
        cluster.ResourceIndexGrain(first).GetGrainId().ShouldNotBe(
            cluster.ResourceIndexGrain(second).GetGrainId());
    }
    ...
    static ResourceId Address(Guid tenant, Guid subscription, string name) => new(
        tenant,
        subscription,
        "prod",
