...
///     budget — and an attacker spread across four identity replicas gets four times the free
///     sign-up codes. That is the entire reason docs/plan/10 § Request pipeline says
///     <i>Redis-backed</i>. It is here because it exercises the same interface with the same
///     semantics — including the honest <c>Retry-After</c> — so a test of the <i>limiting</i> does
///     not need a container, and because a <c>dotnet run</c> with no Redis should still refuse a
///     runaway loop.
/// </remarks>
public sealed class InMemoryRateLimitCounters(IClock clock) : IRateLimitCounters {
