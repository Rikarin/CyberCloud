...
///         the platform → connection edge and logs it. <b>This is the single place tenancy is
///         enforced by code rather than by key</b>, and it is called out here so nobody has to
///         discover it."</i>
///     </para>
///     <para>
...
public sealed class ClusterConnectionTenancyTests(KubeTestCluster cluster)
{
    ...
    static Guid NewClusterId() =>
        Guid.Parse(FormattableString.Invariant(
            $"cccccccc-0000-0000-0000-{Interlocked.Increment(ref next):D12}"));

    static ClusterConnectionDescriptor Descriptor(Guid clusterId, Guid owner) => new()
    {
        ClusterId = clusterId,
        ...
        try
        {
            ...
            lines.ShouldContain(
                x => x.Contains("user:ops-7f3c", StringComparison.Ordinal)
                    && x.Contains(clusterId.ToString("D", CultureInfo.InvariantCulture), StringComparison.OrdinalIgnoreCase),
    ...
    public async Task ADeniedEdgeRaisesASecurityEvent()
    {
        ...
        var errors = LogCapture.Lines
            .Where(x => x.Level >= Microsoft.Extensions.Logging.LogLevel.Error)
            .Select(x => x.Message)
            .ToList();
    ...
    public async Task AClientCallIsRefusedBecauseNothingElseChecksIt()
    {
        ...
        outcome.IsFailure.ShouldBeTrue(
            "a client is not a tenant and must not be treated as the owner.");
    ...
    public async Task EveryGrainMethodChecksTheOwningTenant()
    {
        ...
        typeof(IClusterConnectionGrain)
            ...
            .Length
            .ShouldBe(8, "the interface has eight methods; the probe covers whatever is there.");

        var unchecked_ = await cluster.Reacher(TenantB).ProbeUncheckedMethodsAsync(
            clusterId,
            Descriptor(clusterId, TenantB),
            CommandFor(TenantB));

        unchecked_.ShouldBeEmpty(
            "these methods did not check the owning tenant: " + string.Join(", ", unchecked_)
