...
///     ⚠
///     <b>
///         The failure class: a long <see cref="Operation{T}" /> poll must not die at minute
///         eleven.
///     </b> docs/plan/11 § Protocol makes access tokens ten minutes long, and docs/plan/09's
...
public sealed class CredentialsNeverLeakTests {
    ...
    [InlineData(
        ...
        """{"error":"invalid_client","error_description":"The client credentials are wrong."}"""
    )]
...
public sealed class ChainedCredentialTests {
    ...
    public async Task A_failed_credential_stops_the_chain() {
        ...
        var thrown = await Should.ThrowAsync<AuthenticationFailedException>(async () => await chain.GetTokenAsync(
                new TokenRequestContext([CyberCloudScopes.Default]),
                Cancel.Token
            )
        );
    ...
    public async Task An_exhausted_chain_names_every_credential_it_tried() {
        using var chain = new ChainedTokenCredential(
            new Unavailable("no CLI on the path"),
            new Unavailable("no federated token file")
        );

        var thrown = await Should.ThrowAsync<CredentialUnavailableException>(async () => await chain.GetTokenAsync(
                new TokenRequestContext([CyberCloudScopes.Default]),
                Cancel.Token
            )
        );
