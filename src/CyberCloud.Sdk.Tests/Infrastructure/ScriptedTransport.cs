...
public sealed class ScriptedTransport : HttpMessageHandler {
    ...
    public ScriptedTransport(Func<HttpRequestMessage, int, HttpResponseMessage> script) {
        this.script = script;
    }
    ...
    protected override async Task<HttpResponseMessage> SendAsync(
        ...
        throw new InvalidOperationException(
            $"The transport script ran out after {index} request(s): {request.Method} {request.RequestUri}."
        );
    }
...
public static class Responses {
    ...
    public static HttpResponseMessage Operation(
        string state,
        IEnumerable<(string Step, string Message, int Percent)>? progress = null,
        string? error = null
    ) {
