...
///         ⚠
///         <b>
///             <see cref="Purged" /> is a list of purges <i>accepted</i> and not of purges
///             finished.
///         </b> A purge is a long-running operation like every other write on this
...
public sealed record ExpirySweep {
    ...
    ///     ⚠
    ///     <b>
    ///         This is the reconcile half, and it is the reason the sweep asks the index before it
    ///         asks the manager.
    ///     </b> The registry's invariant is
    ///     <i>
    ///         an entry exists only while the index
    ///         says <see cref="IndexEntryState.SoftDeleted" />
    ///     </i>, and
    ...
    ///         ⚠
    ///         <b>
    ///             Reported because a reminder is otherwise unobservable from outside the grain, and
    ///             "armed exactly while there is something to sweep" is a claim rather than a comment
    ///             only if something can read it.
    ///         </b> Orleans exposes <c>GetReminder</c> to the grain and
    ...
    ///     ⚠
    ///     <b>
    ///         The rotating cursor <see cref="IExpirySweeperGrain.MaxPerSweep" /> describes, and it
    ...
    ///         that is over the cap can see the window moving rather than infer it.
    ///     </b> It is the first
...
///         That section decided the fork —
///         <i>
///             "the purge splits, and there is no system
///             principal"
///         </i> — and built both fronts; what it recorded as still missing was
///         <i>
///             "the
///             caller of the mechanism … Nothing yet drives <c>PurgeExpiredAsync</c> on a clock; it is a
///             method with two tests and no scheduler."
///         </i> This is that caller and it is nothing else:
...
///         ⚠
///         <b>
///             THE RECORDED SHAPE WAS A REMINDER PER PARKED RESOURCE, AND IT IS RE-TAKEN HERE
///             RATHER THAN FOLLOWED — because the premise it rested on stopped being true two commits
///             before this one.
///         </b> docs/plan/07 § Azure RBAC and docs/plan/08 § Soft delete both say
///         the same thing in the same words: a reminder
///         <i>
///             "registered when the resource is parked
...
///             while a reminder needs only the moment the window is opened."
///         </i> Issue #71 built that
...
///             ⚠
///             <b>
///                 A reminder that fires at the deadline is a second durable copy of the
///                 deadline.
///             </b> Its due time <i>is</i> <c>IndexEntry.RecoverableUntil</c>, written into
...
///             against a caller-side comparison — <c>SoftDeleteAsync</c> takes a duration
///             <i>
///                 "so
///                 that one activation stamps the window and reads it"
///             </i> — is an argument against
...
///                 ⚠
///                 <b>
///                     THIS REASON WAS STATED TOO WIDELY AND THE NARROWING IS OWED TO IT
///                     (2026-09-05, #12 review).
///                 </b> A lost <i>group-level</i> row is not repaired by
...
///         ⚠
///         <b>
///             A GRAIN OF ITS OWN AND NOT A REMINDER ON THE REGISTRY, WHICH IS A CYCLE RATHER THAN
///             A PREFERENCE.
///         </b> A sweep calls <c>PurgeExpiredAsync</c>, and
...
///         declined a system principal because
///         <i>
///             "a system principal is a subject that passes every
///             check, and this document has no way to bound one"
///         </i>. Nothing changes here: the sweep
...
///         ⚠
///         <b>
///             What it does inherit is every refusal the authorized front inherits, and the lock is
///             the one worth naming.
///         </b> A <c>CanNotDelete</c> lock stops a clock-driven purge exactly as
...
///         restored. docs/plan/07 § Azure RBAC:
///         <i>
///             "a clock that overruled it would make the lock
///             mean 'until the platform disagrees'"
///         </i>.
///     </para>
///     <para>
...
///         to type the purge. ⚠
///         <b>
///             Neither is an argument for holding every window open for
///             ever
///         </b> — that trades a disk for a name and a quota held permanently — and neither is
///         this grain's to fix.
///     </para>
/// </remarks>
...
public interface IExpirySweeperGrain : IGrainWithStringKey {
    ...
    ///         ⚠
    ///         <b>
    ///             THE SENTENCE ABOVE IS ONLY TRUE BECAUSE <see cref="ArmAsync" /> REGISTERS AT MOST
    ///             ONCE, AND IT WAS FALSE WHEN THIS FILE FIRST SHIPPED (fixed 2026-09-05, #12 review).
    ///         </b>
    ...
    ///         ⚠
    ///         <b>
    ///             A cap because a pass holds this activation, and <c>ListRequest.MaxPageSize</c>
    ///             because the platform has already argued that number.
    ///         </b> One entry costs an index read
    ...
    ///         ⚠
    ///         <b>
    ///             THE CAP TAKES A ROTATING WINDOW AND NOT A FIXED PREFIX, WHICH IS A CORRECTNESS
    ///             FIX RATHER THAN A REFINEMENT (2026-09-05, #12 review).
    ///         </b> This remark used to say the
    ...
    ///         ⚠
    ///         <b>
    ///             Called by every writer that <i>adds</i> a registry entry, which is what makes
    ///             "armed while there is something to sweep" true rather than aspirational.
    ///         </b> There are
    ...
    ///         ⚠
    ///         <b>
    ///             REGISTERED AT MOST ONCE, AND THIS PARAGRAPH USED TO SAY THE OPPOSITE WAS
    ///             HARMLESS (2026-09-05, #12 review).
    ///         </b> It read
    ///         <i>
    ///             "Orleans'
    ///             <c>RegisterOrUpdateReminder</c> is itself idempotent on (grain, name), so a second call
    ///             re-registers the same row rather than adding one"
    ///         </i>. Idempotent on the row's
    ...
    ///         out, and a resource group whose deletes arrive more often than hourly
    ///         <b>
    ///             never swept
    ///             at all
    ///         </b>. The implementation now registers only when
    ...
    ///         ⚠
    ///         <b>
    ///             <see cref="AlwaysInterleaveAttribute" />, because both callers are on a request
    ///             path and a sweep is deliberately long.
    ///         </b> <see cref="MaxPerSweep" /> purge
    ...
    ///         ⚠
    ///         <b>
    ///             A silo with no reminder service succeeds anyway, and that is the same decision
    ///             <c>ResourceGroupGrain.ArmOrDisarmAsync</c> takes for the two-phase-create reaper.
    ///         </b>
    ...
    ///     ⚠
    ///     <b>
    ///         This exists because <see cref="ExpirySweep.Disarmed" />'s remarks complain that it
    ///         should.
    ///     </b> They say a reminder "is otherwise unobservable from outside the grain", and
    ...
    ///     reason it did not was that nothing asked. It answers the operator's question —
    ///     <i>
    ///         is
    ///         anything actually going to end this group's windows?
    ///     </i> — and it is what lets
    ...
    ///         ⚠
    ///         <b>
    ///             THE BACKFILL, AND IT EXISTS BECAUSE ARMING FROM THE WRITE PATH ALONE COVERS ONLY
    ///             THE WINDOWS THAT OPEN AFTER IT IS DEPLOYED (2026-09-05, #12 review).
    ///         </b>
    ...
    ///         design argument on this interface makes to a per-resource reminder —
    ///         <i>
    ///             "if the
    ///             registration is lost … the resource is left exactly where master leaves it today"
    ///         </i> —
    ...
    ///         ⚠
    ///         <b>
    ///             It is what <c>ExpirySweeperBackfill</c> calls, once per resource group, and
    ///             nothing else may.
    ///         </b> The condition is the whole point: arming unconditionally would
    ...
    ///         ⚠
    ///         <b>
    ///             The registry's failure is propagated here and not swallowed, unlike
    ///             <see cref="ArmAsync" />'s.
    ///         </b> "Could not read the registry" is not "the group has
    ...
    ///         ⚠
    ///         <b>
    ///             Public rather than only reachable from the reminder, for the reason
    ///             <c>IResourceGroupGrain.ReapOrphansAsync</c> is:
    ///         </b> a sweeper nobody can run is a
    ...
    ///         ⚠
    ///         <b>
    ///             It leaves the group armed if it found anything, and stands down only if it did
    ///             not.
    ///         </b> That is the same arm-or-disarm decision
