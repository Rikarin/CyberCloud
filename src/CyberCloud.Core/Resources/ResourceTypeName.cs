...
public readonly record struct ResourceTypeName {
    ...
    public static Result<ResourceTypeName> Create(string? providerNamespace, string? typePath) {
        ...
        return problem is null
            ? Result<ResourceTypeName>.Success(new(providerNamespace!, typePath!))
    ...
    static string? DescribeNamespace([NotNullWhen(false)] string? value) {
        ...
        for (var i = 0; i <= value.Length; i++) {
            ...
            if (!IsValidSegment(value.AsSpan(start, i - start))) {
                return "'"
                    + value
                    + "' is not a valid provider namespace: segment "
                    + Int(segments + 1)
                    + " ('"
                    + value[start..i]
                    + "') must be 1-"
                    + Int(MaxSegmentLength)
                    + " characters of [A-Za-z][A-Za-z0-9]*. A namespace "
    ...
    static string? DescribeTypePath([NotNullWhen(false)] string? value) {
        ...
        for (var i = 0; i <= value.Length; i++) {
            ...
            if (!IsValidSegment(value.AsSpan(start, i - start))) {
                return "'"
                    + value
                    + "' is not a valid resource type: segment "
                    + Int(segments + 1)
                    + " ('"
                    + value[start..i]
                    + "') must be 1-"
        ...
        return segments <= MaxDepth
            ...
            : "'"
            + value
            + "' is not a valid resource type: it nests "
            + Int(segments)
            + " levels deep and the limit is "
            + Int(MaxDepth)
            + ". See the remarks on "
