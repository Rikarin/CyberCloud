...
public static class ResourceNaming
{
    ...
    static string? Describe(string? name)
    {
        ...
        for (var i = 0; i < name.Length; i++)
        {
            ...
            if (!allowed)
            {
                return DescribeCharacter(c) + " at position " + Int(i)
    ...
    static string Explain(string? name, string kind, string problem)
    {
        ...
        return shown + " is not a valid " + kind + ": " + problem + ". A " + kind + " is "
            + Int(MinLength) + "-" + Int(MaxLength) + " characters and must match " + Pattern
            ...
            + "becomes a Kubernetes object name and a label value, which caps at " + Int(MaxLength)
