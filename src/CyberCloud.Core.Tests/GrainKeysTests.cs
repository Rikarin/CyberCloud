...
public class GrainKeysTests {
    ...
    ///     derivation: every member but <see cref="GrainKeyKind.None" /> is a key.
    /// </summary>
    [Fact]
    public void TheClosedSetHasTwentySixShapes() =>
        Enum.GetValues<GrainKeyKind>().Count(x => x != GrainKeyKind.None).ShouldBe(26);
}
