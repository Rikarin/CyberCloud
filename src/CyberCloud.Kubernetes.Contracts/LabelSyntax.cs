...
public static class LabelSyntax
{
    ...
    public static Result ValidateValue(string? value, string key)
    {
        ...
        return problem is null
            ...
            : Result.Failure(
                ...
                + "A label value is at most " + Int(MaxValueLength) + " characters, must begin and "
                + "end with a letter or a digit, and may contain '-', '_' and '.' in between "
                + "(regex: " + ValuePattern + "). An object carrying an illegal label is rejected by "
    ...
    public static Result ValidateKey(string? key)
    {
        ...
        return problem is null
            ...
            : Result.Failure(
                ...
                + "DNS-subdomain prefix and a '/', then a name of at most " + Int(MaxNameLength)
                + " characters beginning and ending with a letter or a digit (regex: "
                + NamePattern + "). The prefix is lower-case only and at most "
                + Int(MaxPrefixLength) + " characters.");
    }
    ...
    static string? DescribeValue(string? value)
    {
        ...
        {
            return "it is " + Int(value.Length) + " characters long and the limit is "
                + Int(MaxValueLength);
        }
    ...
    static string? DescribeName(string name, string what)
    {
        ...
        return name.Length > MaxNameLength
            ? what + " is " + Int(name.Length) + " characters long and the limit is "
                + Int(MaxNameLength)
    ...
    static string? DescribePrefix(string prefix)
    {
        ...
        {
            return "its prefix is " + Int(prefix.Length) + " characters long and a DNS subdomain is "
                + "capped at " + Int(MaxPrefixLength);
        }
        ...
        for (var i = 0; i <= prefix.Length; i++)
        {
            ...
            {
                return "its prefix segment '" + new string(segment) + "' " + problem
                    + ". A key prefix is a DNS-1123 subdomain (regex: " + PrefixPattern
    ...
    static string? DescribePrefixSegment(ReadOnlySpan<char> segment)
    {
        ...
        {
            return "is " + Int(segment.Length) + " characters and a subdomain segment is capped at "
    ...
    static string? DescribeAlphanumericBounded(string value)
    {
        for (var i = 0; i < value.Length; i++)
        {
            ...
            if (c is not (>= 'a' and <= 'z' or >= 'A' and <= 'Z' or >= '0' and <= '9'
                or '-' or '_' or '.'))
            {
                return Describe(c) + " at position " + Int(i)
