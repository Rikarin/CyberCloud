...
/// <remarks>
///     <para>
///         ADR-013 (docs/plan/02): <i>"<c>Build()</c> and <c>Apply()</c> exist only on the
///         fully-qualified interface, so <b>an unlabelled object does not compile</b>."</i>
///         docs/plan/09 § The command builder says the same: <i>"The requirement from the brief —
...
public sealed class CompileFailureTests
{
    ...
    public void TheHarnessCompilesAgainstTheRealContractsAssembly()
    {
        ...
        References.ShouldContain(
            x => x.Display != null && x.Display.EndsWith(Path.GetFileName(contracts.Location), StringComparison.OrdinalIgnoreCase),
