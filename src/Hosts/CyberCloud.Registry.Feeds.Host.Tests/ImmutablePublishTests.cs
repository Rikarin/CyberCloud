...
public sealed class ImmutablePublishTests {
    ...
    public async Task ALosingClaimRemovesItsOwnBytesAndLeavesTheWinners() {
        ...
        (await context.Catalogue.PutAsync(winner, false)).IsSuccess.ShouldBeTrue();
        ...
        (await store.GetAsync(context.StoragePrefix + winner.StoredAt, Token)).IsSuccess.ShouldBeTrue(
            "the winner's bytes were removed"
        );
        (await store.GetAsync(context.StoragePrefix + loser.StoredAt, Token)).IsFailure.ShouldBeTrue(
            "the loser's bytes were left behind"
        );
    }
        ...
        public Task<Result<FeedEntry>> PutAsync(FeedEntry entry, bool replace) {
            if (!replace && entries.ContainsKey(entry.Path)) {
                return Task.FromResult(
                    Result<FeedEntry>.Failure(
                        ErrorCode.ResourceAlreadyExists,
                        $"'{entry.Path}' is already in this feed."
                    )
                );
            }
        ...
        public Task<Result<ImmutableArray<FeedEntry>>> ListAsync(string pathPrefix) =>
            Task.FromResult(
                Result<ImmutableArray<FeedEntry>>.Success(
                    [.. entries.Values.Where(x => x.Path.StartsWith(pathPrefix, StringComparison.Ordinal))]
                )
            );
