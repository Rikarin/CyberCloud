...
/// <remarks>
///     <para>
///         ⚠ <b>Against a SUBSTITUTED <c>IResourceGraphQuery</c>, so these prove three things and
///         no fourth</b>: that stage 6 admits the shape under its reserved namespace and refuses
...
public sealed class ResourceGraphQueryRoutingTests {
    ...
    public async Task APostWithAQueryReachesTheQueryServiceWithTheTokensCallerAndNotTheResourceManager() {
        ...
        var response = await gateway.SendAsync(
            ...
            gateway.Token(GatewayHarness.TenantA, subjectId: "alice"),
    ...
    public async Task ThePageIsRenderedInTheCollectionEnvelopeWithItsColumnsAndANextLinkThatCarriesThePageParameters() {
        ...
        root.GetProperty("columns").EnumerateArray().Select(x => x.GetProperty("name").GetString()).ShouldBe(["name", "location"]);
        ...
        root.GetProperty("nextLink").GetString().ShouldBe(
    ...
    public async Task ALastPageOmitsNextLinkRatherThanWritingItEmpty() {
        var gateway = new GatewayHarness();
        gateway.Graph.OnQuery = _ => Result<ResourceGraphQueryPage>.Success(new() { Columns = [new("Count", "long")], Rows = ["""{"Count":3}"""] });

        var response = await gateway.SendAsync("POST", Address(GatewayHarness.TenantA), gateway.Token(GatewayHarness.TenantA), body: Body("resources | count"));
    ...
    public async Task ANextLinkIsFollowedByPostingTheSameBodyAndTheUrlsPageParametersAreRead() {
        ...
        var followed = await gateway.SendAsync(
            ...
            Address(GatewayHarness.TenantA),
            gateway.Token(GatewayHarness.TenantA),
            query: $"api-version={OneTypeRegistry.TheVersion}&$top=2&$skipToken=2.0123456789abcdef",
            body: Body()
        );
        ...
        var overridden = await gateway.SendAsync(
            ...
            gateway.Token(GatewayHarness.TenantA),
            query: $"api-version={OneTypeRegistry.TheVersion}&$top=2&$skipToken=2.0123456789abcdef",
            body: Body(top: 7, skipToken: "7.fedcba9876543210")
        );
    ...
    public async Task AnotherTenantsAddressIsNotFoundAndTheServiceIsNeverAsked() {
        ...
        var response = await gateway.SendAsync("POST", Address(GatewayHarness.TenantB), gateway.Token(GatewayHarness.TenantA), body: Body());

        response.Status.ShouldBe(StatusCodes.Status404NotFound, response.Body);
        gateway.Graph.Requests.ShouldBeEmpty("stage 3 refuses a tenant that disagrees with the token before anything is queried");
    }
    ...
    public async Task AnyVerbButPostIs405WithAllowPost(string method) {
        ...
        var response = await gateway.SendAsync(method, Address(GatewayHarness.TenantA), gateway.Token(GatewayHarness.TenantA), body: method is "PUT" or "PATCH" ? Body() : "");
    ...
    public async Task ABodyWithoutTheQueryIs400ThatSpellsTheShape(string body) {
        ...
        var response = await gateway.SendAsync("POST", Address(GatewayHarness.TenantA), gateway.Token(GatewayHarness.TenantA), body: body);
    ...
    public async Task AServiceRefusalIsTheServicesSentenceAs400() {
        var gateway = new GatewayHarness();
        gateway.Graph.OnQuery = _ => Result<ResourceGraphQueryPage>.Failure(ErrorCode.InvalidRequestBody, "'mv-expand' is not in the resource graph's KQL subset.");

        var response = await gateway.SendAsync("POST", Address(GatewayHarness.TenantA), gateway.Token(GatewayHarness.TenantA), body: Body("resources | mv-expand tags"));
    ...
    [InlineData("/tenants/{t}/subscriptions/cccccccc-0000-0000-0000-000000000003/providers/CyberCloud.ResourceGraph/resources")]
    [InlineData("/tenants/{t}/subscriptions/cccccccc-0000-0000-0000-000000000003/resourceGroups/prod/providers/CyberCloud.ResourceGraph/resources")]
    ...
    public void TheQueryIsCountedAsAReadByTheRateLimiter() {
        ...
        GatewayRouter.Classify(GatewayHarness.ResourcePath(GatewayHarness.TenantA) + "/restart", "POST", query).ShouldBe(RequestClass.Write);
    }
}
