...
///         ⚠
///         <b>
///             This is the implementation <see cref="ICallerContextResolver" />'s remarks asked the
...
///         a JWKS change and never a gateway deploy. The issuer and audience (item 2) are pinned from
///         <see cref="GatewayIdentityOptions" />. <c>tid</c> (item 3), <c>sub_typ</c> (item 4) and
...
///         of kind <c>server_error</c>, and this resolver rethrows it as an
///         <see cref="InvalidOperationException" /> naming the issuer. A <c>401</c> there would tell
///         every caller their credential is bad while the gateway's own dependency is down — and
...
///         <see cref="OpenIddictValidationService" /> is registered scoped and this resolver is a
///         singleton, because <c>AuthenticateStage</c> is; the request's <c>RequestServices</c> is
///         the scope OpenIddict expects to be resolved from.
///     </para>
/// </remarks>
...
sealed class JwksCallerContextResolver(IClock clock, ILogger<JwksCallerContextResolver> logger)
    : ICallerContextResolver {
    const string BearerPrefix = "Bearer ";

    /// <inheritdoc />
    public async Task<Result<TokenClaims>> ResolveAsync(
        HttpRequest request,
        ...
        if (header.Length == 0) {
            return Result<TokenClaims>.Failure(GatewayErrors.Unauthenticated("no Authorization header"));
        }

        if (!header.StartsWith(BearerPrefix, StringComparison.Ordinal)) {
            return Result<TokenClaims>.Failure(
                GatewayErrors.Unauthenticated("the Authorization header is not a bearer token")
            );
        }

        var validation = request.HttpContext.RequestServices.GetRequiredService<OpenIddictValidationService>();

        ClaimsPrincipal principal;

        try {
            principal = await validation.ValidateAccessTokenAsync(header[BearerPrefix.Length..], cancellationToken);
            ...
            return Result<TokenClaims>.Failure(
                GatewayErrors.Unauthenticated("the bearer token was not issued by this platform or has expired")
            );
            ...
            throw new InvalidOperationException(
                ...
                + "key set could not be retrieved. CyberCloud:Gateway:Identity:Issuer names the host "
    ...
    /// <summary>
    ///     Reads the gateway's claims off a principal the validator has already accepted.
    /// </summary>
    ...
    ///         needs the identity host; reading one does not. <c>JwksCallerContextResolverTests</c>
    ...
    ///         <c>/.well-known/cybercloud-token-policy</c> with the instruction that the gateway
    ...
    ///     </para>
    /// </remarks>
    internal static Result<TokenClaims> ToClaims(ClaimsPrincipal principal, DateTimeOffset now) {
    ...
    static Result<TokenClaims> Refuse(string reason) =>
        Result<TokenClaims>.Failure(GatewayErrors.Unauthenticated(reason));
}
