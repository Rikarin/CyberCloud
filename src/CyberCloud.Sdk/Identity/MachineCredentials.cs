...
public abstract class TokenEndpointCredential : TokenCredential, IDisposable {
    ...
    protected async ValueTask<AccessToken> RedeemAsync(
    ...
    internal static AccessToken ToAccessToken(TokenPayload payload) {
        if (string.IsNullOrEmpty(payload.AccessToken)) {
            throw new AuthenticationFailedException(
                "The identity server returned a token response with no access_token."
            );
        }
...
///     ⚠
///     <b>
///         The secret is held in a field and reaches exactly one place: the form of the token
///         request.
///     </b> It is not in <see cref="object.ToString" />, not in any exception this type
...
///     </para>
///     <para>
///         docs/plan/11 § Credentials schedules the certificate row for M2 as
///         <i>
///             "mTLS for service
///             principals"
///         </i>. ⚠ This is <c>private_key_jwt</c>, not mTLS, and the difference is worth
...
public sealed class CertificateCredential : TokenEndpointCredential {
    ...
    internal string CreateAssertion(string audience) {
        ...
        var signingInput =
            $"{Base64Url.Encode(Encoding.UTF8.GetBytes(header))}.{Base64Url.Encode(Encoding.UTF8.GetBytes(payload))}";
        var signature = Sign(Encoding.ASCII.GetBytes(signingInput));

        return $"{signingInput}.{Base64Url.Encode(signature)}";
    }

    static string JsonEncodedAudience(string audience) =>
        JsonSerializer.Serialize(audience, SdkJsonContext.Default.String);
...
///         docs/plan/11:
///         <i>
///             "No secret is ever stored, on either side. … it removes an entire incident
///             class."
///         </i> So this credential caches nothing across processes: the projected token is
...
public sealed class WorkloadIdentityCredential : TokenEndpointCredential {
    ...
    public override async ValueTask<AccessToken> GetTokenAsync(
        ...
        try {
            ...
            throw new CredentialUnavailableException(
                $"The projected service-account token at '{tokenFilePath}' could not be read.",
                e
            );
        }
