// Bootstrap — docs/plan/09 § The platform's own cluster, part 2 of the phase-3 answer:
...
// ROT."
//
// ── ⚠ WHY THIS IS A TARGET OF ITS OWN AND NOT ONLY THE FIRST PHASE OF `E2E` ──────────────────────
//
// It was only that phase until issue #25. Doc 09's sentence was aspirational: nothing in the build
// invoked `bootstrap.sh`, so the only thing keeping it working was that nobody had changed it.
// Making it the first phase of `E2E` fixed that on paper — the half that needs no cluster runs on
// every invocation, including the ones that go on to block — and then nightly.yml's four hostile-BYO
// legs and its kind cluster showed the other half of the problem: `E2E` is BLOCKED on a suite that
// does not exist (`test/CyberCloud.E2E`), so every job whose real content was this dry-run ended red
// on a precondition that had nothing to do with the dry-run, and .github/scripts/assert-bootstrap-
// exercised.sh had to exist to read the pass out of the log of a job that always failed. A red job
// cannot get redder: the day the manifests stop applying to a restrictive-PSA cluster, that job goes
// from red to red.
//
// So the dry-run is a target a workflow can call for what it is. `E2E` still runs it first — the
// phase is unchanged and doc 09's sentence still holds — and `./build.sh Bootstrap --kube-context X`
// is green exactly when the seven preflight cases and the dry-run against X are, red when either is
// not, and never for a reason that lives in a different row of docs/plan/23 § Test layers.
//
// It depends on nothing. The script is bash and kubectl; `Compile` would add minutes to a job whose
// whole point is that it needs no .NET beyond the build host itself. artifacts/images.json is read
// when `Images` has left one and a placeholder digest is used when it has not — SiloImageReference
// says why that is a warning and not a block.
...
partial class Build {
    ...
    [Parameter(
        "kubectl context for the bootstrap dry-run — Bootstrap, and the first phase of E2E. Absent skips the cluster half, not the whole phase."
    )]
    ...
    ///         ⚠
    ///         <b>
    ///             Every case below is chosen to die before the script's first cluster call, and
    ///             that is a safety property rather than a stylistic one.
    ///         </b> <c>bootstrap.sh</c>'s
    ...
    void ExerciseBootstrap() {
        ...
        Refuses(
            bash,
            "no --image",
            $"{BootstrapScript} --context {unresolvable} --shards {shards}",
            "--image is required"
        );
        ...
        Refuses(
            bash,
            "no --shards",
            $"{BootstrapScript} --context {unresolvable} --image ghcr.io/x@sha256:0",
            "--shards is required"
        );
        ...
        Log.Information(
            "Bootstrap: bootstrap.sh preflight exercised — 7 case(s), each pinned to its own message, no "
    ...
    void BootstrapDryRun(Tool bash, AbsolutePath fallbackShards) {
        if (string.IsNullOrWhiteSpace(KubeContext)) {
            Log.Warning(
                "Bootstrap: the bootstrap dry-run against a cluster was SKIPPED — no --kube-context. Only "
        ...
        Log.Information("Bootstrap: bootstrap.sh --dry-run clean against context {Context}", KubeContext);
    }
    ...
    string SiloImageReference() {
        if (!ImageManifestFile.FileExists()) {
            Log.Warning(
                "Bootstrap: {Manifest} does not exist — ./build.sh Images has not run, so the dry-run uses "
