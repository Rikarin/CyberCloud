...
partial class Build {
    ...
    ///         ⚠
    ///         <b>
    ///             <c>conformance.yaml</c> is checked for presence and for naming the same resource
    ///             type, and nothing more.
    ///         </b> The suite that reads it lives in <c>test/</c> and does not
    ...
    void CheckManagedChartFiles(AbsolutePath chart, ChartMetadata metadata, List<string> failures) {
        if (!chart.ToString()
                .StartsWith(
                    ManagedChartsDirectory + Path.DirectorySeparatorChar.ToString(),
                    StringComparison.Ordinal
                )) {
            return;
        }
    ...
    ///         ⚠
    ///         <b>
    ///             The schema covers every key, including the <c>@internal</c> ones, and marks them
    ///             rather than dropping them.
    ///         </b> This file is Helm's contract as well as ours: Helm
    ...
    ///         ⚠
    ///         <b>
    ///             Keys are sorted ordinally and <c>x-cybercloud-order</c> carries declaration
    ///             order.
    ///         </b> Sorting is what makes the file independent of any dictionary iteration
    ...
    static JsonObject ObjectNode(IReadOnlyList<ChartValue> children, bool freeform, string? inherited) {
        ...
        var node = new JsonObject { ["type"] = "object", ["additionalProperties"] = freeform };
    ...
    ///         ⚠
    ///         <b>
    ///             Exclusion is written on every descendant here and may be written only once in
    ///             values.yaml.
    ///         </b> The two rules point opposite ways on purpose. In the source, a repeated
    ...
    /// </remarks>
    static string LineFeeds(string text) => text.Replace("\r\n", "\n", StringComparison.Ordinal);
    ...
    static ValueAnnotation? TakeAnnotation(
        ...
        if (duplicated is not null) {
            problems.Add($"{duplicated.Skip(1).First().Line}: `@{duplicated.Key}` appears twice on '{key}'.");
        }
    ...
    ///     ⚠
    ///     <b>
    ///         This used to be a 100 ms match timeout restating <c>SchemaProperty.PatternTimeout</c>,
    ///         and it went the same way that member did (#76).
    ///     </b> A .NET match timeout is wall clock, so
        ...
        foreach (var value in values) {
            ...
            switch (annotation.Type) {
                ...
                case "boolean" when literal is not LiteralKind.Boolean:
                    problems.Add(
                        $"{value.Line}: '{value.Name}' is declared `{{boolean}}` and is not `true` or `false`."
                    );
                    break;

                case "integer" when literal is not LiteralKind.Integer:
                    problems.Add(
                        $"{value.Line}: '{value.Name}' is declared `{{integer}}` and its value is not a whole number."
                    );
                    break;

                case "number" when literal is not (LiteralKind.Integer or LiteralKind.Number):
                    problems.Add(
                        $"{value.Line}: '{value.Name}' is declared `{{number}}` and its value is not a number."
                    );
                    break;
            ...
            if (annotation.Choices.Count > 0 && annotation.Type is "object" or "boolean") {
                problems.Add(
                    $"{value.Line}: `@enum` on a `{{{annotation.Type}}}`. It applies to strings, numbers and arrays."
                );
            }
            ...
            if (annotation.Minimum is not null) {
                if (annotation.Type is not ("integer" or "number")) {
                    problems.Add(
                        $"{value.Line}: `@range` on a `{{{annotation.Type}}}`. It applies to integers and numbers."
                    );
                } else {
            ...
            if (annotation.Widget is not null && annotation.Type is "object" or "array") {
                problems.Add(
                    $"{value.Line}: `@widget` on a `{{{annotation.Type}}}`. A widget renders one scalar field."
                );
            }
    ...
    static void CheckRange(ChartValue value, ValueAnnotation annotation, List<string> problems) {
        ...
        if (minimum > maximum) {
            problems.Add(
                $"{value.Line}: `@range {annotation.Minimum}..{annotation.Maximum}` is empty — no value satisfies it."
            );
            return;
        }
    ...
    /// </summary>
    /// <remarks>
    ///     ⚠ The default is checked for the same reason <see cref="CheckRange" /> checks it:
    ///     <c>
    /// helm
    ///     lint --strict
    ///     </c> validates <c>values.yaml</c> against the <c>values.schema.json</c> this
