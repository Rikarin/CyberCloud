...
static class ExtensionCommands {
    ...
    static Command Add(CycHost host, GlobalOptions globals, VerbTreeDocument tree) {
        var source = new Option<string>("--source") {
            Description = "The executable to install. A local path — cyc does not download extensions.", Required = true
        };
        ...
        var force = new Option<bool>("--force") { Description = "Replace an extension of the same name." };
    ...
    ///     ⚠
    ///     <b>
    ///         Nothing installed prints an empty list; something broken never prints as though it
    ///         worked.
    ///     </b> Every entry carries a <c>state</c>, and anything that is not <c>ok</c> also
    ...
    static Command List(CycHost host, GlobalOptions globals, VerbTreeDocument tree) {
        ...
        command.SetAction(parse => {
                ...
                foreach (var directory in unsafeDirectories) {
                    invocation.Console.Note(
                        $"⚠ '{directory}' is writable by more than you, so cyc will run no extension at all."
                    );
                }
                        ...
                        Payload.Object(
                            [
                                ...
                                new KeyValuePair<string, Payload>(
                                    "source",
                                    record.Source is null ? Payload.Null : Payload.Text(record.Source)
                                ),
                                new KeyValuePair<string, Payload>(
                                    "installed",
    ...
    static string StateOf(
        ...
        if (file.Length != record.Size
            || !string.Equals(ExtensionStore.HashOf(file.FullName), record.Sha256, StringComparison.Ordinal)) {
            return "modified";
        }
