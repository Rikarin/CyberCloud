...
/// <summary>
///     <c>cyc config</c> — docs/plan/21 § Decisions:
///     <i>
///         "<c>~/.cyc/config</c> with named profiles;
///         every setting also an env var (<c>CYC_SUBSCRIPTION</c>, …) for CI."
///     </i>
/// </summary>
static class ConfigCommands {
    ...
    static Command Get(CycHost host, GlobalOptions globals, VerbTreeDocument tree) {
        ...
        var command = new Command("get", "One setting's value, resolved through the environment and the profile.") {
            key
        };
    ...
    static Command Set(CycHost host, GlobalOptions globals, VerbTreeDocument tree) {
        ...
        command.SetAction(parse => {
                ...
                invocation.Console.Note(
                    $"Set {parse.GetRequiredValue(key)} in profile '{invocation.Settings.Profile}'."
                );
                            ...
                            .. settings.OrderBy(x => x.Key, StringComparer.Ordinal)
                                .Select(x => Payload.Object(
                                        [
                                            ...
                                            new KeyValuePair<string, Payload>(
                                                "env",
                                                Payload.Text(CycSettings.VariableFor(x.Key))
                                            )
                                        ]
                                    )
                                )
    ...
    static Command UseProfile(CycHost host, GlobalOptions globals, VerbTreeDocument tree) {
        ...
        var command = new Command("use-profile", "Choose the profile used when --profile and CYC_PROFILE are absent.") {
            name
        };
    ...
    static Command Telemetry(CycHost host, GlobalOptions globals, VerbTreeDocument tree) {
        ...
        var command = new Command("telemetry", "Whether cyc may send usage telemetry. Off unless you turn it on.") {
            state
        };
        ...
        command.SetAction(parse => {
                ...
                if (!string.Equals(choice, "status", StringComparison.Ordinal)) {
                    TelemetryConsent.Record(
                        host,
                        invocation.Settings,
                        string.Equals(choice, "on", StringComparison.Ordinal)
                    );
                ...
                invocation.Render(
                    Payload.Object(
                        [
                            new KeyValuePair<string, Payload>(
                                "telemetry",
                                Payload.Text(TelemetryConsent.IsEnabled(invocation.Settings) ? "on" : "off")
                            ),
                            new KeyValuePair<string, Payload>(
                                "asked",
                                Payload.Boolean(TelemetryConsent.WasAsked(invocation.Settings))
                            )
                        ]
                    )
                );
