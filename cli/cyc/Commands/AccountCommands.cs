...
///     <c>expiresOn</c>, camel-cased. It also branches on the exit code:
///     <b>
///         3 means "not signed in,
///         run cyc login"
///     </b> and anything else means something different went wrong, so mapping a missing
...
static class AccountCommands {
    ...
    static Command Set(CycHost host, GlobalOptions globals, VerbTreeDocument tree) {
        ...
        var command = new Command("set", "Change the current profile's subscription or tenant.") {
            subscription, tenant
        };
    ...
    static Command List(CycHost host, GlobalOptions globals, VerbTreeDocument tree) {
        var command = new Command(
            "list",
            "The profiles in ~/.cyc/config. For the platform's subscriptions, see 'cyc rest'."
        );
    ...
    static Command GetAccessToken(CycHost host, GlobalOptions globals, VerbTreeDocument tree) {
        ...
        command.SetAction(async (parse, cancellationToken) => {
                var invocation = CycRunner.Bind(host, globals, tree, parse);
                var scopes = parse.GetValue(scope) is { Length: > 0 } requested
                    ? requested
                    : [CyberCloudScopes.Default];
