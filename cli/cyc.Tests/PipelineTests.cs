...
/// </summary>
/// <remarks>
///     ⚠ docs/plan/21 § Grammar calls <c>cyc rest</c>
///     <i>
///         "the escape hatch for anything not yet a
///         verb"
///     </i>. Raw means untyped, not unauthenticated: the tests below read the headers off the
...
public sealed class PipelineTests {
    ...
    public async Task ARestBodyThatIsNotJsonIsAUsageError() {
        ...
        (await host.RunAsync("rest", "--uri", "/x", "--method", "POST", "--body", "{oops")).ShouldBe(
            (int)ExitCode.Usage
        );
    }
